...
- **「出ていない」と「別のものが出ている」は違う。** v7.0 では作者の
  「押し倒され文章が出ない」を字義どおり受け取って読み、拘束が付いていないと断じたが、
  実測すると本文は **91%** のフレームで出ていて拘束も **2.24本** あった。
  本当の穴は「本文の相手が、実際に四肢を取っている魔物だったフレームが **0%**」。
  **症状の言葉ではなく、症状の数を測ること**
- **捕獲を「beginCapture が呼ばれた回数」で数えてはいけない。** `beginCapture` は
  `G.mode!=='battle'` で即 return するが、`h.recCapBy` には前の捕獲のものが残っている。
  v7.0 で観測フェーズに `enemiesUpdate` を回した途端、そこから
  `hurtHero → checkStaminaCollapse → beginCapture` が毎フレーム呼ばれ、
  同じ一件を数え続けて **98 → 3061** に膨れた。
  数えるなら **`B.captures` が実際に増えた時だけ**
- **`B.time` を進めるのは `battleTick` だけ。** その外で拍を回す所(観測フェーズなど)では
  時計が止まる。`limbFree`(再拘束の猶予)・`lineCd`・`ailRateT` は全部 `B.time` で
  「◯秒後」を置くので、**その時刻が永遠に来ない**——v7.0 では掴みの成功率が
  **0%**(39669回呼んで13回)になっていた。**止めるのは数字であって時計ではない**
- **`else if` の順序で死ぬ帯ができる。** `if(bd>24) 寄せる / else if(bd<=44) 掴む` は
  24〜44px で絶対に掴まない。`else if` の鎖は「広い条件を後ろに」(v6.9 のボス枝と同じ罠)
