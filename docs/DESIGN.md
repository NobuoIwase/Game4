...
直し: **下手人の帳面**(`markCulprit` / `culpritTop`)。
掴んで稼いだ時間(`CULP_HOLD` /秒)と削った分(`CULP_DMG` /ダメージ)の両方を積み、
とどめの名は「その夜いちばん効かせた魔物」にする。ボス・光線・催眠の帰属はこれまで通り優先。
同点に近ければ(`CULP_NEW_K` 以内)、その子がまだ読んでいない相手を先に出す——
★ただし**効かせていない魔物を持ち上げることはしない**(嘘にならない範囲の対策)。
