...
- **壁の形(formations)**: `pillarHall`(柱を5〜9本、r1.1〜2.1)/`escarp`(曲率を持つ崖線16〜28タイル、2か所に坂)/`rockfall`(半径 3.0→0.8 と落ちる5つの塊)/`spine`(同軸に3〜5個の細長い塊)/`chambers`(4〜7×3〜5の部屋を2〜3個、四辺の中央に扉)/`constriction`(向かい合う膨らみ)。`F.wall` が 'brick' なら部屋主体、'flesh' なら狭窄主体、'rock' なら柱と崖と崩落。生えた場所は `formSpots` として `usedF` に半径2で登録し(通路は切り拓いてよい)、名前のある形(pillars/escarp/chamber)は `feat.list` にも入れて台詞・ミニマップに出す。
...
- **検証(run_v32.js)**: `map`(全8階層: 外周の厚みが 2〜12 で場所ごとに違う・外への漏れ0・到達できない床0・床率0.69〜0.72・階層ごとの形の顔ぶれ・袋小路は口を塞ぐと祠へ到達不能=一方通行・巣窟は口を塞ぐと最奥へ到達不能=入口ひとつ・口から奥まで480〜608px)、`den`(役割はフレイラが入りルミナが待つ・待ち役は301フレームすべて澱みの中・二人の最大距離 横394/縦287・最奥到達・番人起床・踏み込み: 押し倒された瞬間に役割が解けて「もう むり! いくね!」、544px→143px まで詰める)。
