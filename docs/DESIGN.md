...
### 3-71. v6.6g オート指揮の二つの偏り

作者の報告:「プレイヤーAIがコストの重い小淫魔を連続で出しまくってモンスターの集団
という感じにならない」「媚薬霧や淫紋の魔法陣などが的確すぎて、モンスターがまともに
近くにいないのに発情して絶頂する」。**どちらも台帳で裏が取れた。**

**(1) 小淫魔が一晩のENの 13.5% を独りで食っていた**
  ...
  ゴブリンは 65回出して 130EN。小淫魔は **10回で 120EN**。
  v6.6 で指揮官に作り替えた時にコストを 8 へ上げたのに、
  枝は「場に居なければ出す」のままだった——**HP20 が死ぬたび出し直していた**。
  いまは **部下(雑魚)が `IMP_NEED_FODDER` 体以上いる時だけ、`IMP_RECALL` 秒の間を置いて**呼ぶ。
  指揮官は部下が居てこそ、という建てつけに合わせた。→ EN上位10種から消えた。

**(2) 置きが的確すぎた** — 媚薬の霧壺の、彼女からの距離:
  ...
  `chooseNightItem` は `add('mist', p.x, p.y, 4)` ——**毎回きっちり足元**に置いていた。
  夜の側は「そこに居そうな所」しか読めない、という建てつけなので、置きは外れるべきだ。
  狂いの幅は相手の速さで決まる: 走行中 `ITEM_MISS_WALK` / 立ち止まり `ITEM_MISS_STAND` /
  組み伏せ中 `ITEM_MISS_HELD`(押さえているのだから、よく当たる)。

### 3-72. v6.6h 押し倒し本文が「汎用っぽい」の中身

作者の報告:「フレイラの敗北文も汎用っぽい感じする」。**フレイラだけの問題ではなかった。**
押し倒せる 38 種で数えると:
...
**四人とも汎用に落ちる 11種**に、**浅い階でいちばん多く会う顔ぶれが丸ごと入っていた**——
ナメクジ・ゴブリン・ゴースト・粘スライム。序盤はほぼ毎回おなじ文を読むことになる。
「汎用っぽい」という体感は、覆っている割合ではなく**会う頻度**の側から来ていた。

52本を書き下ろして、四人とも 38/38(ヤミコの渦の眠り手だけは原理上不要)。

### 3-73. v6.6i 蒸発の学習は、誰のものか

作者の報告二つ:「フレイラの『覚えた』をルミナが発言した気がする」
「フレイラが蒸発を全然学習しない」。**同じ一箇所から出ていた。**

**(1) 学習の台詞を、居合わせた全員が言っていた**

  噴き出した霧は `evapBreathe` が**範囲内の全員**に吸わせる(それは正しい)。
  ところが `chokeTick` の締めで、全員が `feat.evapLearn`
  ——「……もう、ここでは やかない」——を言っていた。

    実測(6夜): feat.evapLearn → フレイラ 1回 / ルミナ 1回

  火を持たないルミナが「やかない」と言うのは、**誰の学習でもない**。
  `evapBreathe` に「焚いた本人か」(`chokeMine`)を渡し、
  本人だけが学習の行を、他の子は新しい `feat.evapOther`(むせただけ)を言う。

**(2) 巣窟・澱みの蒸発にだけ、学習のゲートが無かった**

  沼のほうは `dryAuraTick` に「一度こぼしたら、触れる前に自分で炎を落とす」があり、
  実測でも**学習が立った後の沼の蒸発は 0回**。効いている。
  ところが同じ関数の数行下、`dryEvapCheck`(巣窟・澱み)は**素通り**だった:

    ★巣窟・澱みの蒸発 : 1回 / そのうち学習が立った後 **1回(100%)**
    META.gen.dryLesson : 沼で 1 まで上がり、その後 **2 へ**——上げたのは巣窟側

  そして「媚薬が蒸発した」の banner が出るのは**こちら**なので、
  プレイヤーから見えるのは学習していない側だけだった。
  沼と同じ形に揃えた(`denHazeNear` で判定を一本化し、学習後は炎を落として
  `feat.dryDropDen` を言う)。

**(3) おまけ**: `feat.dryNoMire` の呼び出しが 6夜で **1219回**。
  `sayLine` の間隔で潰れてはいたが、判断のたびに呼ぶ必要は無い。呼ぶ側にも間隔を持たせた。

### 3-74. v6.7 「罠だけで負けている」を測って、出どころを分けた

作者の報告:「プレイヤーAI設置の罠が強い。魔物はまともに寄って来れないのに
これだけで敗北してる」「えちえちエリアに入って負けるシーンが多すぎる」
「基本はヒロイン優勢であってほしくて、それをなんとか崩して欲しい」。

**測ったこと(6夜・階層1/3/5/7/9/11・二人)**

  発情/敏感化を浴びた瞬間の、いちばん近い魔物までの距離
  ...
  雲の出どころ: 魔物(`enemiesUpdate` の雄臭・霧スライム・ガス玉)が湧いた数の
  **92.4%**、夜側の設置(媚薬の霧壺)は **0.6%**、巣窟の仕掛けは 0.5%。

  → **「魔物が居ないのに発情する」は起きていない。** ただし霧壺は
  雲の 0.6% で発情の 7.8%・敏感化の 12.6% を入れている
  ——**一本あたり約13倍濃い**。体感「置きが的確すぎる」は正しく、総量では主役ではない。

**では「えちえちエリアで負ける」は何だったか**

  捕獲21件のうち、巣窟の中で捕まったのは **2件(10%)**。中で負けてはいない。
  負けているのは**外**で、その時の状態は「敏感化100・発情Lv3」を抱えたまま。

  ★**ここで一度、狭い数え方をして間違えた。** 最初「巣窟の仕掛けが入れた発情は
  全体の 0.1%」と書いたが、それは `denTick` が `spawnCloud` した雲だけを
  出どころで数えたもので、床の手・魔法陣・沼・番人・奥の一撃が全部抜けていた。
  **居た場所で数え直すと:**

    巣窟の中に居た時間 6.3% / そこで入った発情 30.5%(毎秒 25.1)
    外 毎秒 3.9  →  **巣窟は外の 6.4倍濃い**

  作者の「えちえちエリアが強すぎる」は当たっていた。
  `DEN_POW0` 0.58→0.46 / `DEN_POW_K` 0.048→0.042 / `DEN_POW_MAX` 1.28→1.10
  (外との比は5倍前後を残す——危ない場所であることは変えない)。

    敏感化の自然減衰 `SENSIT_DECAY` = 1.1/秒 → 振り切れた 100 が抜けるのに **91秒**

  巣窟は「入ると負ける場所」ではなく「**出てから90秒、次の魔物に潰される体にする場所**」
  だった。

**入れたもの: 息を整える間(`calmT` / `calmK`)**

  `condTick` の末尾で、次の全部が揃っている間だけ `calmT` を積む:

    雲の外 / 押し倒されていない / 絶頂・時間停止・魅了拘束でない /
    肢を掴まれていない / 嗅ぎ発作でない / 泉にも灯篭にも居ない /
    巣窟の中でない / `zoneFear(zone) < 2` /
    **`CALM_R`(200px)以内に生きた魔物が居ない**

  `CALM_WARM`(3秒)かけて `calmK` が 0→1 になり、そのぶん抜けが速まる:

    敏感化 1.1 → 1.1+`CALM_SENS`(4.0) = 5.1 /秒
    発情ゲージ 1.5 → 1.5+`CALM_HEAT`(6.0) = 7.5 /秒

  **★触られている間の目盛りは一切変えていない。** プロファイルの
  「まだ抗っている心が快感に軋んでいく過程」は攻められている最中の話で、
  ここはその外側——誰にも触られていない時間の話。
  変わるのは「引きずるかどうか」だけで、巣窟に入るリスクは残る。

  **★速い抜けが効くのは「振り切れた上の帯」だけ**(`CALM_SENS_FLOOR` 35 /
  `CALM_HEAT_FLOOR` 40)。下地はいつもの 1.1 / 1.5 で抜ける。
  帯を区切らずに試した48日のA/Bでは、狙いには効いた代わりに**エロが薄くなりすぎた**:

    | | 降下 | 負けた日 | 到達 | 絶頂/日 | 巣窟の奥へ |
    | --- | --- | --- | --- | --- | --- |
    | A(なし) | 30日 63% | 16日(捕獲8+二連敗8) | 世代6/7階/3人 | 3.2回 | 90回 |
    | B(あり・帯なし) | 34日 71% | 14日(7+7) | 世代7/8階/4人 | **1.3回** | **54回** |

  火照らないので「もういいや」の枝が出ず、**巣窟へ入る回数まで4割減った**。
  直したいのは「91秒引きずること」だけで、火照りそのものではない。

  **計器の穴(二つ目)**: `afterEnd()` を通すと `G.mode` は `'result'` になるので、
  捕獲の日が `'result'` に化けて「捕獲 0%」に見えていた。
  二連敗リセットの日も負けた日なので、`降下 / 負けた日(捕獲+リセット)` で数える。

**霧壺(夜側の設置)** 一本の雲を r80/life9 → **r70/life7.5**(面積×時間 -32%)。
  `NIGHT_ITEM_LIFE.mist` も 7.5 に揃える(HUDの残り時間がずれる)。
  淫紋の魔法陣は発情の 1.0%・敏感化の 1.3% しか入れていないので触っていない。

**計器の話** 60日の通し(`run_loop66`)が出していた「14日の足踏み」は**計器の穴**だった。
観測フェーズ(`B.after`)はプレイヤーを待つので、`afterEnd()` を呼ばないと
捕獲の日に `endBattle` が走らない=**二連敗リセットが発火しない**。
`run_loop67` は日を必ず閉じ、A(息を整える間なし)/B(あり)を同じ地図の種で回す。

### 3-75. v6.7 武器を三人に3種ずつ足した(枠5に対し5種ちょうどだった)

作者の報告:「ヒロインの武器も種類が少ないというか、ルミナがほぼ全ダメージソース
みたいになってる感じあるし」。

**前半は当たっていて、後半は測ると外れていた。**
  ...
  | 2人 | 66.9% | 33.1% | — | — |
  | 3人 | 45.3% | 8.9% | 45.8% | — |
  | 4人 | 41.2% | 4.7% | 45.1% | 8.9% |

  フレイラの 4.7% は階層で振れているだけだった(階別に測ると 8%/30%/57%/22%/19%/26%/46%)。
  間合い内に居る割合は **94.5%** で三人中いちばん高く、動けない時間は
  **10.3%** でいちばん低い。位置取りで損はしていない。**ルミナ偏重は数字に出ない。**
...
**本当の穴は枠だった**
  ...
  三人は毎回まったく同じ構成になっていた。足したのは火力ではなく**選ぶ余地**。

**足した9種**(`UPG` に `owner` 付き / `HEROES[id].wps` / `dpsEst` の `BASE`)

  | 誰 | id | 名前 | すること |
  | --- | --- | --- | --- |
  | フレイラ | `fbrand` | 焼き印 | 体力のいちばん残っている一体に印。`e.brandT/brandK` が `damageEnemy` で読まれ被ダメ +40%。倒れると押し直す。熱も溜まる |
  | | `fash` | 熾火の轍 | `EMBER_STEP`(26px)歩くごとに `zones` へ `ember:true`(= `fire:true`)。熱が満ちていると最大1.8倍長持ち |
  | | `fveil` | 陽炎の衣 | `p.shimmer` を立て、`hurtHero` が被ダメを最大 -45%、`ebullets` を確率で逸らす。ベタベタを乾かす。触れた相手をじりじり焼く |
  | クウ | `ipike` | 氷柱 | `B.pikes` に立てる柱。`pikesTick` が魔物だけ押し返す(ヒロインは通れる)。押されるほど削れて砕ける |
  | | `ithaw` | 霜解けの息 | 範囲内の味方の敏感化 -9 / 発情ゲージ -14 / 粘液とベタベタを落とす。祭壇・呪いの下限は割らない |
  | | `imirror` | 氷鏡 | 味方に `h.iceMirror` を配り、`hurtHero` から `iceMirrorBack` が受けた分の 55% を殴った相手へ凍らせて返す |
  | ヤミコ | `dgrasp` | 闇の手 | いちばん**遠い**動ける一体を `GRASP_PULL`(78px)引き寄せて `GRASP_HOLD` 止める。奥から撃ってくる相手を間合いへ落とす |
  | | `ddrain` | 闇の吸い上げ | `p.drainK` を立て、`damageEnemy` が与ダメの 5.5% を `drainPool` へ。6溜まるごとに**いちばん薄い子**へ回す |
  | | `dsteal` | 影盗み | 範囲内の味方の掴まれた肢を一つ `detachLimb` でもぎ取り、掴んでいた個体を削って止める |

  **この9種は進化(`EVOS`)を持たない。** 覚醒(Lv6〜8)は効く。
  「進化する3種を伸ばすか、進化しない代わりに噛み合う札を並べるか」も選択にする。

  常時効いている札(陽炎の衣・氷鏡・闇の吸い上げ)は、`weaponsUpdate` が
  `atkMult<=0` で早期に return する所で**必ず切る**。切らないと、掴まれて
  武器が止まっても護りだけが残り続ける。`iceMirrorBack` は配り手が
  連れて行かれていたら鏡を外す。

### 3-76. v6.8 画面を横持ちで固定する

作者の指定:「画面サイズは固定にしてしまおう。横持ちメイン。chromeの上のバーを
非表示にできるならその方が画面が大きくなるのでして。正方形の画面の場合も
画面の横のサイズが最大になるように下にボタンとか配置して。元々の画面サイズと
サイズがおなじデスクトップだけは画面ちょっと小さくして」。

**(1) 視界を 960x540 に固定**

  v1.10〜v6.7 は「見える広さ(960x540 相当)は保ったまま、縦横比だけ端末に合わせる」形で、
  端末ごとに `W`/`H` が変わっていた(実測: 800x600 の窓で `W=834 H=620`)。
  カメラの追従・パーティの手綱・ミニマップの位置が全部その上に乗っていたので、
  端末によって「見えている世界」が違った。`VIEW_W=960 / VIEW_H=540` に固定。

**(2) レールは「はっきり横長」だけ(`RAIL_MIN_AR=1.25`)**

  v6.7 は `iw>=ih*0.92` でレールを立てていたので、正方形の窓でも左右に
  132px ずつ取られ、盤面の幅が痩せていた。作者の指定どおり、
  正方形・縦長では `portrait`(ボタンは下)にして幅を最大に使う。

    1180x540 → rails      canvas 769x433
    760x760  → portrait   canvas 749x421   ← 幅をほぼ使い切る
    430x900  → portrait   canvas 424x238
    1440x900 → rails      canvas 1000x562  ← デスクトップだけ 0.90 倍

**(3) 窓が画面いっぱいのデスクトップだけ縮める(`DESK_SHRINK=0.90`)**

  `deskFullWindow()` = 触る端末でない かつ `innerWidth >= availWidth*0.97` かつ
  `innerHeight >= availHeight*0.86`。★縮めるのは **rails の時だけ**——
  正方形・縦長では横幅で頭打ちなので、縮めると「横を最大に」に逆らう。

**(4) ブラウザのバーを消す**

  - `manifest.json`(`display:fullscreen` / `orientation:landscape`)+
    `mobile-web-app-capable` / `apple-mobile-web-app-capable` —— ホーム画面に置いた時
  - 触る端末では**最初のタップで一度だけ** `fsEnter()`(全画面API + 可能なら横向き固定)
  - それ以外は右上の **⛶**(`#fsbtn`。`body.canfs` の時だけ出る)
  - 縦持ち(`ih>iw*1.15`)では `#rotate` で「横向きにすると大きくなります」

  ★`fullscreenchange` で `resize()` を呼び直す。呼ばないと canvas が古い寸法のまま残る。

**(5) 見つけたCSSの穴**: `#handrow.dense .hcard`(id入り・詳細度 1,2,0)が
`body.rails .hcard`(0,2,1)より強く、**レールの札の寸法指定が効いていなかった**。
札が10枚以上ある時だけ起きるので、実プレイでは常に起きていた。
`body.rails #handrow .hcard` に直して詳細度を上げた。

### 3-77. v6.8 画面外に出ない / 戦いの音で駆けつける / 相談を見えるようにする

**(1) 画面の外に出ない — 手綱を「カメラの箱」で測る**

  実測(三人・8夜・934秒): **誰かが画面の外に居た時間 8.0%**、最大 177px はみ出し。
  二人の開きの最大は **851px** で、手綱 `PARTY_MAXDX=740` を超えていた。

  原因は手綱の測り方。カメラは `!out || captive` の子たちの**重心**を追うのに、
  手綱は**二人ずつの距離**で掛かっていた。三人以上では、二人ずつが 740 以内でも
  重心から半画面(480)より外に出られる。視界が固定になったので箱で書ける:

    partyClamp(): 重心から |dx| <= W/2 - VIEW_KEEP, |dy| <= H/2 - VIEW_KEEP
    viewPull()  : 箱の 74% を超えたら、歩く向きにも重心へ戻る力を混ぜる

  ★掴まれている子・捕まった子は動かせないので、動ける子のほうが寄る。

**(2) 戦いの音 — 掴まれる前に駆けつける**

  実測: 仲間が掴まれている時間のうち、**誰かが助けに動いていたのは 31.6%** だけ。
  残りはジェム拾い 14.5% / 逃げ 12.5% / 引き撃ち 8.8%。二つ穴があった:

  - `COVER_TH:1.15` が高すぎた。`distressOf` は肢一本 0.55・二本 1.10 なので、
    **押し倒し(1.7)まで待たないと誰も来ない**。→ **0.60** に下げる
  - `assist` の枝が `flee` と `kite` の**下**にあった。自分が引き気味の時は
    仲間が掴まれていても行かない。→ 窮地(`ASSIST_HARD=1.20`)なら
    逃げ・引き撃ちより**先**に置く枝を足した

  そして作者の言葉どおり「音を聞いたり見たりで」気づく形にした:

    fightNoise(h): 直前に殴られた(FIGHT_HURT 秒) / 230px に FIGHT_N 体 → FIGHT_W ずつ
    heardBy(p,h) : HEAR_R(820px)以内なら壁越しでも聞こえる / 見えていれば遠くても

  掴まれていなくても「戦っている音」だけで駆けつける理由になる。

**(3) 相談が見えない — 門番を二つに分ける**

  実測(三人・4夜): 決め直しのうち集合を呼べたのは **4.2%(1/24)**。内訳:

    もう集まっている            37.5%
    ×話す価値なし(PARTY_TALK_CD) 37.5%
    ×危ない(partyDanger)        16.7%  ← 全部『150px内に魔物』
    ×集合の間隔(GATHER_CD)        4.2%
    ○集合を呼ぶ                  4.2%
  ...
  **直した後(三人・8夜・1373秒)**
  ...
  | | 前 | 後 |
  | --- | --- | --- |
  | 誰かが画面の外に居た時間 | 8.0%(最大177px) | **0.0%**(最大16px) |
  | 二人の開きが半画面超え | 23.7%(最大851px) | **1.2%**(最大698px) |
  | 掴まれている間に誰かが助けに動いていた | 31.6% | **54.2%** |
  | 向き合って相談していた時間 | 2.2% | **7.6%** |
  | 集合の呼びかけ → 成立 | 15→6 (40%) | **40→20 (50%)** |

  ★残った目盛り: 仲間が掴まれている間の `retreat` はまだ 35.4% ある。
  `ASSIST_HARD` を下げれば更に助けに行くが、自分の身を捨てる方向でもあるので
  ここで止めてある(実プレイの手触りで決める)。

  - `partyDanger()` は 150px 以内に魔物が一体でも居れば真。洞窟ではほぼ常に真なので、
    **歩み寄る**ほうの門番を `partyBusy()`(掴まれ・押し倒し・体力4割・96px内の魔物)に分けた。
    **立ち止まって向き合う**ほうは今までどおり厳しく見る
  - `PARTY_TALK_CD` 10→5
  - ★いちばん多い形は「**もう揃っている所で決め直す**」(37.5%)なのに、そこでは
    間隔に阻まれて何も言わずに歩き出していた。揃っていて案が割れたなら
    それがまさに相談の場面なので、間隔を待たずに言い合う(`nearTalk`)

### 3-78. v6.9 作者の七点

**(1) レーザー触手の条は「動かない線」だった**

  作者の言葉:「現在はヒロインに完全に追尾してる、本来の意図は
  『ビームは動かずに壁に当たり、ヒロインはあまり通りたくない』という感じ。
  横や縦や斜めにランダムに出て、それは動かない」。

  実装は毎フレーム彼女を狙って `RAY_TURN`(0.5 rad/秒)で向きを変えていた——追尾そのもの。

    rayPickAngle(e,ray): 点く前に 8方位のどれかへ固定する
      ・すぐ壁に当たる向き(L<120)は選ばない(線にならない)
      ・同じ触手の他の条と 0.2rad 以内で被る向きは選ばない
      ・★いま彼女が居る向きは減点(RAY_AIM_AVOID)——狙い撃ちにしない
      ・残りはランダム
    raytentTick: 条が一本でも出ている間は動かない(動くと線ごと引きずられる)
    RAY_ON 2.2→4.2 / RAY_OFF 3.4→2.6(地形として読ませる)
    RAY_TURN は削除

**(2) 水鏡の女王が実質討伐不可だった**

  `exposed` の条件は「半径420の鏡タイルのうち 45%以上が `dryT`(炎で乾いた)か
  `iceT`(氷で凍った)」。割る手はフレイラの乾燥オーラ(スタミナ25%以下で焚く)か
  クウの氷河(`ibloom` Lv5 + `ward` Lv2 の進化)しか無い。
  **二人が居ない周回・進化が出ていない周回では、物理的に本体へ触れない。**

  三つ目の道を足した——**映り身を払い切る**。この一体の芯(映り込みでしか触れない)
  をそのまま使う: 映すものが尽きれば、面は保たない。

    MIRROR_SHARD 6 + 世代×1.5(上限16)体の映り身を倒す
      → MIRROR_EXPOSE_T(9秒)だけ本体が出る → 沈む
    killEnemy に mirrorling のフックを置き、女王の e.shards を積む

  ★この四つの定数を最初 `MONSTERS` の中に書いてしまい、`MONSTERS` の走査
  (`m.deep=d`)が「数値にプロパティを付けた」で落ちて **data.js が途中で死んでいた**。
  `check_dead.py` は「定義はあるが読まれない」だけを見ていたので素通りだった
  ——**逆向きのチェック(読んでいるのに BAL に無い鍵)を足した**。

  そして、その新しいチェックが**既存のバグを一つ見つけた**:
  `js/render.js` が v5.2 で `MRING_T` に改名される前の名前を読んだままで、
  `undefined - R.t = NaN` になり、菌輪の「閉じる瞬間にふくらむ」が死んでいた。

  ★**そして本当の原因はもっと手前にあった。** `enemiesUpdate` の分岐の順番で、
  汎用の `}else if(e.boss){` が **v6.0 で足したボス3体より前**に書かれていたので、
  三体の専用挙動が**一度も呼ばれていなかった**:

    mirrorqueen  → mirrorqueenTick が走らない = exposed が一度も立たない
                   = damageEnemy が必ず「鏡に阻まれた」で返る = **完全に無敵**
    nevermet     → 忘れ水の霧を撒かない
    firstslug    → 与えたダメージで濃くならない

  作者の「実質討伐不可」は、実質どころか**文字どおり不可**だった。
  三つの分岐を汎用の前へ移した。
  ★教訓: `else if` の鎖に汎用の枝(`e.boss` のような属性判定)があるなら、
  **個別の枝はその前にしか置けない**。新しいボスを足す時は必ず順番を見ること。

**(3) すぐ隣の宝箱を取らない**

  拾い判定が `22px` の決め打ちだった。彼女の半径が14前後あるので、
  ほぼ中心を踏まないと開かない。実測(三人8夜): 取り残した箱47個のうち
  **50px以内まで寄ったのに開かなかった箱が3個**、残り43個は 120px より遠い
  (=そもそも寄っていない・通常)。

    CHEST_TAKE_R 26 + 本人の半径の半分  …… 箱の肩に触れれば開く
    CHEST_GRAB_R 90 / CHEST_GRAB_K 0.55 …… 目当てが別でも、道すがら寄る

  ★目当ての系は箱を「行き先」としてしか見ないので、行き先が別に決まっていると
  すぐ隣の箱が視界から消える。歩く向きに混ぜるだけなら往復にならない。

**(4) えちえちエリアへ無策で突っ込む / 一瞬でスタミナ切れ**

  作者の疑問「えちえちエリアではスタミナ切れにならない設定になってるの?」への答え:
  **そんな設定は無い。** 実測で巣窟の中のスタミナは **毎秒 4.90 減**(外は 2.08)
  ——**2.4倍速い**。「一瞬で」の正体は連続絶頂の代金
  (`CLIMAX_STAM_COST` 12 × (1 + `CHAIN_COST_K` 0.9 × n)。3回目だけで 33.6)。
  そして `PIN_STAMINA_TH` 35 を割ると、掴まれた時に**押し倒される**。
  巣窟 → 連続絶頂 → スタミナ35未満 → 押し倒し → 捕獲、が一本道だった。

  「無策」の最後の一枚は、入るかどうかの確率式の `+(aroused?0.25:0)`
  ——**火照っているほど入りやすい**。そこに二つ足した:

    DEN_VETO_STAM 0.38 / DEN_VETO_HEAT 3 …… ここを割っていたら確率を回さずに諦める

**(5) 攻略の思考(支度してから入る)**

  作者の言葉:「例えばクウがいるなら通る所を大変だけど凍らせて通るとか、
  そういう攻略に対するヒロインの思考が欲しい」。

  境での選択肢を、いままでの二つ(入る / 諦める)から**三つ**にした:

  | 支度 | 条件 | すること |
  | --- | --- | --- |
  | `ice` | クウが居て、境までの道が `DEN_PREP_WET`(30%)以上濡れている | クウが `kuuIce` で道を凍らせる。他は境で待つ |
  | `rest` | 誰かのスタミナが `DEN_PREP_STAM`(55%)未満 | 境から離れて息を整える |
  | `cool` | 誰かの発情Lvが `DEN_PREP_HEAT`(2)以上 | 冷ましてから |
  | `gather` | 散っている | 揃ってから |

  `denPrepPlan()` が計画を立て、`denPrepDone()` が済んだかを見る。
  済むまで `DEN_PREP_STEP` ずつ迷いを延ばし、状態は `plan`(「支度してから入る」)。
  `iceTick` は、この計画がある時は普通の橋(`iceBridge`)より**先に**褥の前の道を凍らせる。

**(6) フレイラの蒸発は一度きりだった**

  作者の報告:「たまにえちえちエリアで発動したりした後、そのまま移動しても
  霧が強くならない。えちえちエリアの媚薬溜まりは危険なものなのだから、
  罰としてもっと凶悪にして欲しい」。

  `dryEvapCheck` は `A.evap` のフラグで**一つのエリアにつき一度だけ**噴いていた。
  「煮え」を **炎のエリアではなく蒸発そのもの**に紐づけた(`B.boil` + `boilTick`):

    DRY_BOIL_T 16秒のあいだ、DRY_BOIL_CD 1.1秒ごとに霧が噴く
    DRY_BOIL_RATE 2.4(通常のガスより濃い) / DRY_BOIL_R 150 / DRY_BOIL_LIFE 9
    煮えた時間で濃さが上がる(DRY_BOIL_STACK 1.35 まで)
    ★火を入れた子が褥・澱みの中を歩いているなら、歩いた先が煮える(×1.15)
    煮えている所に立っている子は DRY_BOIL_HEAT(22/秒)ぶん直に発情する

  ★**炎のエリアに付けた最初の版は動かなかった。** v6.6i の学習
  (「褥だ。ここで焚いたら外まで届く」で自分で炎を落とす)が一拍で効くので、
  14秒のうち一度しか煮えない。**一度火が入った媚薬は、彼女が炎を落としても煮え続ける**
  ——これが「罰」の形で、作者の「そのまま移動しても」への答え。

  ★そしてもう一つ穴があった。**巣窟側の大きな蒸発(`dryEvapCheck`)は、ほぼ発火していない。**
  巣窟は澱みで囲む設計(v3.2)なので、炎のエリアの中にはまず**沼**が入る。
  `dryAuraTick` は沼を先に見るので、`mireEvaporate` が学習を立て、
  次の一拍で彼女が炎を落とし、`dryEvapCheck` まで辿り着かない。
  作者の「**たまに**えちえちエリアで発動したりした後」の『たまに』はこれだった。
  だから煮えは `mireEvaporate` 側からも始める(蒸発した沼が褥・澱みの中なら)。

**(7) クウの加入が早すぎる / 巻き戻りで武器を持って戻る**

  作者の言葉:「8層で加入するイメージだったクウが6層攻略時に加入した。
  難易度を調整するか、あるいは二日連続でやられてヒロインの意思で過去に戻った時は
  武器も引き継ぎにするか」。**両方入れた。**

  - `runReset(wipeKnow=false)`(＝彼女たちの意思で巻き戻った時)は、
    **武器・パッシブ・進化・祈りの積み上げ・今夜の好みを持って戻る**。
    Lv と経験値は残さない(残すと巻き戻りが「ただの続き」になる)。
    ★ループの筋にも合う——巻き戻すのは負けた側で、忘れるのは勝った側。
    彼女たちは覚えているのだから、手に馴染んだ形だけは残る。
    魔核を討った側の巻き戻り(`wipeKnow=true`)では、彼女たちが忘れる側なので何も残らない
  - `PARTY_JOIN` に `deep`(フレイラ4 / クウ8)を足した。`META.deepest`
    (巻き戻しでも消えない「いちばん深く行った階」)と比べる。
    ★浅い所で足踏みし続けても来ないのは別の不幸なので、
    目安は「+2回の巻き戻し」で越えられる
...
### 3-79. v6.10 ノートの Chrome — 画面の基準を「窓」から「盤面」へ

作者の言葉:「macbook airのchromeでも動かせるようにして」。

**動かなかったのではなく、携帯の形でしか正しくならない置き方だった。**
v6.8 で視界を 960x540 に固定した時、レール(左右の札とボタン)も ADV(立ち絵と台詞の箱)も
**窓いっぱい**(`inset:0`)に置いたままにしていた。携帯の横持ちでは盤面が窓をほぼ埋めるので
誰も気づかないが、ノートの 16:10 の窓では盤面が窓の 69% しか使わない——実測 1440x788 で
**盤面 1000x562**。残りは次のように化けていた:

| 症状 | 原因 |
| --- | --- |
| 札が窓の左上の隅に固まり、盤面との間に何も無い帯ができる | レールの grid が窓幅 1440 の三列だった |
| 立ち絵が盤面の上下へはみ出し、暗幕も台詞の箱も枠の外へ落ちる | `#advStand{height:86%}` の 86% が**窓**の 788 だった |
| 13枚のデッキが一列に伸び、下半分がスクロールしないと見えない | 札の寸法が 62/52/47px の三段で、**窓**の縦横で選んでいた |
| ブックマークバーを出し入れすると盤面の大きさが変わる | `deskFullWindow()` の敷居 0.86 を MacBook Air がまたぐ |

**直し方は一つ**——`core.js:boardVars()` が毎 resize で盤面の矩形を CSS へ渡す
(`--bx/--by/--bw/--bh`)。レールと ADV はそれを基準に置く。あわせて三つ:

- **レールは余った余白まで広がる**(`RAIL_MAX=236`)。ノートでは `DESK_SHRINK` で盤面を
  わざと縮めるので、その余白がそのまま札の三列目になる。実測 1440x788 で**レール 212px**
- **札の寸法はレールの実寸から決める**(`fitRailCards`)。三段の決め打ちをやめ、
  「幅 aw・高さ ah に n 枚が収まる一番大きい寸法」を 62px から 40px まで下りながら探す。
  客札が増えて畳が狭くなれば測り直す(`UI.buildHand` から呼ぶ)
- **レールの帯は既定では盤面の帯**。そこに収まらない時だけ窓の余白ぶんまで広げる
  (携帯の横持ちのように盤面が低い時。`--barY/--barH`)

`deskFullWindow()` の敷居は 0.86→`DESK_FULL_H=0.78`。macOS の `availHeight` は
メニューバー(25px)しか引かないのに Chrome 自身のタブ+アドレス欄が 87px、
ブックマークバーを出すと 121px 食う——MacBook Air(875)では 788/875=0.90 と
741/875=0.847 で、**バーの有無だけが敷居をまたいでいた**。

**実測(13枚デッキで出撃・8つの窓)**: 札がスクロールに隠れた量は全部 0、
窓の外へ出た要素も 0、ADV の矩形は 8つ全部で盤面と一致。

| 窓 | 盤面 | レール | 札 |
| --- | --- | --- | --- |
| iPhone 横 844x390 | 556x312 | 136px | 42px×13 |
| iPad 横 1180x820 | 887x499 | 138px | 43px×13 |
| **MacBook Air 13 1440x788** | **1000x562** | **212px** | **62px×13** |
| MacBook Air 15 1710x995 | 1239x697 | 227px | 62px×13 |
| デスクトップ 1920x1040 | 1425x802 | 236px | 62px×13 |

★あわせて `.github/workflows/pages.yml` が `manifest.json` を配っていなかった
(v6.8 で足したのに写し忘れ)。公開ページでは PWA の全画面指定が効いていなかった。

### 3-80. v7.0 敗北の本文 — 誰に / 読める形で / 同じ相手ばかりにならないように

作者の四点:「敗北時の文章の内容が直観的でなくて分かりにくい & 誰に倒されたか分からない、
更に大体羽虫とかワームばかりで他の敗北文がピックアップされない。読みやすさの改善と
敗北文が他のものが出るようにする対策、それと敗北後の観察が現状意味をなしていない
(押し倒されないので押し倒され文章が出ない、モンスターの動きが本来の動きではなく
直線的なものになる、など)」。

**★まず在庫を数えた。足りていた。** capture の本文は MONSTERS 68種すべてに存在し
(専用59 + base継承9)、**default に落ちる組み合わせは 0%**。しかも四人ぶん別々に 239本。
つまり「他の敗北文が出ない」のは**書かれていないからではなく、選ばれていないから**。

#### (1) 誰に倒されたか — 画面のどこにも相手の名が無かった

実測(`run_def70a` / `run_def70c`):

| 測ったもの | 値 |
| --- | --- |
| capture 本文 239本のうち、title に相手の名前が入っているもの | **0 / 239** |
| 本文そのものに相手の名が出てくる回数 | **0回** |
| 「とどめ: 〜」の行 | 結果画面の 51行中3行目・12px・色 var(--body)・太さ400 |
| 二人が捕まった夜の「とどめ」の行 | **1行だけ**(`B.capturedBy` は一つしか無く、最後の一人ぶん) |

`sum.captures[]` には各人ぶんの `by` が入っているのに、見出しは `sum.capturedBy` 一つだけを
読んでいた。直し: **その子ごとの敗北札**(相手の姿 `data-icon` / 名前16px太字 / 階級・系統 /
とどめの理由)を本文の頭に置き、「とどめ:」の平文は札へ吸収。
戦闘中の本文(`drawPinScene`)にも相手の名と「3 / 12」の拍数を出す。

#### (2) 読みやすさ — 8割が箱の外にあった

| 測ったもの | 前 | 後 |
| --- | --- | --- |
| 本文の箱の高さ / 中身 | 200px / 1228px | 373px / 1156px |
| **スクロールの向こうに隠れている割合** | **84%** | **68%**(「▼全部読む」で 0%) |
| 文字 / 行送り | 12px / 22.8px | 13.5px / 27.7px |
| 段落 | `white-space:pre-wrap` の `\n\n` | 本物の `<p>`(下마진14px) |

pin の一行は**短かった**(中央値26字・50字超 0本・箱からはみ出す行 0本)ので、
切れてはいない。問題は**送りの速さ**だった——一律2.6秒だと中央値で**毎分600字**
(日本語の黙読は400〜600字/分の上限)、上位1割(33字)では**毎分762字**で読み切れない。
`AFTER_BEAT_T` の一律送りを廃し、`PIN_BEAT_BASE + 字数/PIN_BEAT_CPS` にした
(0.8 + 26/11 = 3.2秒 ≒ 毎分490字)。長い行には長く置かれる。

#### (3) 相手の偏り — 「多いから」ではなく「掴む手しかとどめになれない」から

30夜の実測(`run_def70b`):

| | |
| --- | --- |
| 敗北本文の相手 | **18種**。吸液羽虫23・地上ワーム22・核の落とし子10(計98)。**上位3種で56% / 上位5種で72%** |
| 捕獲の瞬間、彼女の300px以内に居た魔物 | **45種**。1位 痺れ浮遊子1214(23%)・2位 地上ワーム790・3位 ゴブリン606。吸液羽虫は**6位**(208・3.9%) |
| 押し倒し(pin)が起きた回数 | 30夜で **26回**(種類9) |

★**居る分布と選ばれる分布が一致しない。** いちばん多く彼女を囲んでいる痺れ浮遊子は
とどめに**一度も**出てこない。`beginCapture` の相手が `h.pinBy` か `oldestRestraint(h)`
——**押さえている手**だから。掴まない責め手(浮遊子・眼系・霊系の間接責め)は、
どれだけ彼女を追い詰めても名前が残らなかった。

直し: **下手人の帳面**(`markCulprit` / `culpritPick`)。ここは三回書き直した。

**(a) argmax は逆効果だった。** 最初は帳面の首位をとどめにしたが、首位は毎晩
「長く張り付く硬い手」(石の番兵・沼の触手・核の落とし子)になる。
旧来の「倒れた瞬間に押さえていた手」の方が**偶然が混じるぶん散っていた**——
30夜で 種類 17→**13** / 上位5種 57%→**74%** と、はっきり悪化した。
→ 効かせた分に**比例して引く**形に戻す(未読の相手は `CULP_NEW_K` 倍)。

**(b) 帳面に 1〜3種しか入っていなかった。** `markCulprit` は掴んだ時間と
与えたダメージしか積まない。この作品でいちばん効く責め手——触れずに軋ませる側——が
最初から候補に入っていなかった。順に足した:
- 光線・催眠の源(`h.lastBeam` / `h.lastHypno` は id を覚えている。`CULP_BEAM` / `CULP_HYPNO`)
  → 絶頂照射触手が 3件 → **6件**
- 雲の撒き手。`spawnCloud` の `src` は `'gas'`/`'musk'` の**種別**でしかなく、
  誰が撒いたかを持っていなかった。`mid` を足して 10箇所で渡す(`CULP_GAS`)。
  地形から湧く雲(沼・菌輪・蒸発)は魔物が居ないので null のまま
...
| 種類 | 21 | 23 |
| 捕獲 | 128 | 100 |
| **捕獲100件あたりの種類** | **16.4** | **23.0** |
| 上位3種で | 39% | 38% |
| 上位5種で | 55% | 53% |
| ★`default`(相手が特定できず汎用文) | **12件(9.4%)** | 上位10に出ない(≤4件) |

★**集中度そのものは動かなかった。** 上位3種で 4割・上位5種で 5割強は、
魔物の出現数がそもそも偏っているからで、帳面では変えられない。
動いたのは「捕獲あたりの種類」と、**`default` 落ち**——どの魔物に負けても
同じ汎用文が出ていた 9.4% が、ほぼ消えた。作者の指摘に効いたのはここ。

#### (4) 観測フェーズ — 「出ていない」のではなく「画面と本文が別人だった」

作者の報告は二つだが、実測すると一つは**字義どおりには起きていなかった**:

| 測ったもの(`run_def70b` / `run_def70d`) | 値 |
| --- | --- |
| 押し倒し本文が「描かれる条件」を満たしていたフレーム | **91%** ——出ていた |
| 主の四肢拘束の平均本数 | **2.24本** ——取られてもいた |
| ★**本文の相手が、実際に四肢を取っている魔物だったフレーム** | **0%** |
| 魔物の向きの変化(中央値 rad/frame) | 観測中 **0.0000** / 通常の戦闘 0.0004 |

`afterNextPin` が近くの魔物から**一様ランダムに1体**選び、彼女の隣へ瞬間移動させて
その本文を流していた。だがその魔物は掴まない。実際に押さえているのは捕獲時点から
絡んだままの別の魔物(沼の触手が支配的)。**「押し倒され文章が出ない」の正体はこれ**
——出ていないのではなく、読んでいる本文と目に見えている絵が別人だった。
「直線的」のほうは完全に正しく、毎秒52pxでまっすぐ寄るだけだった。

直し:
- 本文の相手は**実際にその子の四肢を取っている魔物**から選ぶ(`afterPickMon`。
  まだ読んでいない相手を先に)。一本も取られていなければ `AFTER_CALL_R` 内の手を呼んで
  **本当に取らせる**
- 全員に `captive` の帳面を持たせ、`captiveTick`(v6.6e で単独捕獲側に入った仕組み)を
  観測でも回す——責め手の入れ替え・身を捩って一本ほどく・這って逃げてまた取られる
- **`enemiesUpdate` を回す**。魔物が本来の動きをする。
  ★順序が要る: `enemiesUpdate` → `captiveTick`。逆だと寄せた手を同じフレームで
  引き戻してしまい、拘束が 2.24本 → **0.20本** に落ちた
- `nearestHeroIdx` は観測中だけ捕まった子も標的に数える(でないと全員 out なので
  `leaderIdx()` に丸まり、魔物が一人に団子になる)
- ★★**時計を進める。** これが無いと掴みが**一度も成立しない**。
  `freeSlotFor` は v6.2 の「振りほどいたばかりの肢は猶予のあいだ取らせない」を
  `h.limbFree[slot] > B.time` で見るが、`B.time` を進めるのは `battleTick` だけ。
  観測では時刻が捕獲の瞬間で止まったままなので、`captiveTick` が一本ほどくたびに
  置かれる `limbFree = B.time + REGRAB_GRACE` の時刻が**永遠に来ない**——
  全スロットが恒久的に塞がる。実測(`run_def70g`):

  | | 時計を止めたまま | 時計を進めた |
  | --- | --- | --- |
  | `attachMonster` の呼び出し / 成功 | 39669 / **13**(成功率 **0%**) | 1371 / **189**(**14%**) |
  | 付いてから外れるまで(中央値) | 0.00秒(時計が動いていない) | **8.10秒** |
  | 枠が空いていた場面 | 37602 | **189**(ほぼ常に取られている) |

  ★**止めるのは「数字」であって「時計」ではない。** ただし `endBattle` の
  `softK = B.time / ESS_SOFT_T` は逓減の基準なので、**進んだままだと長く観測するほど
  実入りが増える**。観測を終える時に時刻も捕まった瞬間へ戻す(`afterFreezeApply(true)`)
- ★**「観測では何も進まない」を明示的に守る。** v6.5 は `battleTick` を回さないことで
  担保していたが、魔物の拍を回すので `afterSnap`/`afterFreezeApply` で
  オーブ・エッセンス・異常・絶頂・摩耗・XP・`META.life`・`META.codex` を捕まった時点へ戻す
  (図鑑の「見た場面」= `META.readScenes` だけは残す——読んだものは読んだ)

### 3-81. v7.1 作者の三点 — 本文の言葉 / 魔物のバランス / 観測で何が見えるか

作者の続報:

1. 「文章の内容が分かりにくい ← 単純に内容の文章が読みにくい。もっと分かりやすい言葉と
   内容として。動きや喘ぎ声、擬音等を描写して欲しい」
2. 「羽虫とワーム ← なんとかバランスを取って欲しい。特にボスとか、遠距離チクチクの
   淫紋以外は出オチみたいな扱いになってる感じある」
3. 「観察 ← 押し倒し文が出てないように思える。出てるのかな? でも読んだ覚えがない。
   あとヒロインの動きとか抵抗とか、絶頂とかも描写して欲しいかな、普段通りの動きで」

#### (3) 観測 — ★v7.0 の「91%」は、条件の測り違いだった

v7.0 で「押し倒し本文は 91% のフレームで出ていた」と書いた。**これは誤りだった。**
測ったのは `drawPinScene` の**中の条件**であって、**関数が呼ばれているか**ではない。

```js
/* js/render.js draw() ——v6.5 で観測フェーズを足した時、ここが 'battle' のままだった */
if(G.mode==='battle'){ drawCutin(g); drawPinScene(g); }
```

観測フェーズの `G.mode` は `'captured'`。**`drawPinScene` は一度も呼ばれていない。**
作者の「読んだ覚えがない」は、記憶違いでも見落としでもなく、**画面に出ていなかった**。
同じ `G.mode==='battle'` の門が、もう二つ掛かっていた:

| 門(js/render.js) | 観測フェーズで起きていたこと |
| --- | --- |
| `if(G.mode==='battle'){ drawCutin; drawPinScene }` | 本文もカットインも**一度も描かれない** |
| `if(G.mode==='battle'){ pinGauge / charmBindGauge / struggleRing }` | **もがきの輪が出ない**(抵抗が見えない) |
| `moving: p.moving && G.mode==='battle'` | 歩きの絵が**常に止まり絵** |
| `mood = (G.mode==='captured'‖…)?'pinned' : p.climaxT>0?'climax' : …` | `captured` が先に当たるので**絶頂の顔が一度も出ない** |
| `pr = clamp(1-B.captureT/2.8,0,1)`(暗幕) | `B.captureT` は観測に入ると減らない → **55% の暗幕が出しっぱなし** |

さらに、見えるようにしても**動くものが無かった**。`struggleRaw` は `h.pinned` で弾かれる
(押し倒されている間は抗えない)ので、観測のあいだ `h.struggle` は捕獲の瞬間の値で凍る。
輪を出しても針が動かない。`captiveTick` の「身を捩って逃げる」も `slideXY` の**瞬間移動**で、
一フレームで 24px 飛ぶだけ——絵の上では動いていない。

→ 門を四つ開け、暗幕を観測中は 0.42 固定に。そのうえで:

- **`BAL.AFTER_STRUG`(毎秒7.0)** で観測中だけ `h.struggle` を自前に溜める。
  溜まる速さは `1 - aphro/180 - 0.14*heatLv`(下限0.25)——**抗う力が快感に食われていく**。
  満ちたら一本ほどく(それまでの `CAP_SHAKE_T` 12秒の固定間隔は、上限として残す)
- **`BAL.CAP_CRAWL_T`(0.55秒)** で `CAP_SHAKE_D` を這う。`h.moving=true` を立てて
  **普段の歩きの絵**のまま進む。向きも `h.face` に入れる

**実測(`run_v71c` / 5〜10夜を三度 / 観測フレーム 9128・20312・22537。`drawPinScene` を
包んで、中の `g.fillText` が「いま流れている拍の文字列」をそのまま描いた回数を数えた。
★どの拍が流れているかは夜ごとに違うので、三度ぶんの幅で載せる):**
...
| ★歩きの絵で描いたフレーム | **0%** | **7〜12%** |
| ★もがきが増えた(輪の針が動いた)フレーム | **0%** | **64〜69%** |

輪の 88〜100% と絶頂の 26〜36% の幅は、その瞬間に主が拘束されているか・達しているかで
決まる。**0% との差が答えで、幅そのものは夜の中身**。

#### (2) 出オチ — 動かない魔物を、彼女から560px 離れた無作為な方角に出していた

まず測った。16夜・全カード Lv3・全階層(`run_v71d`)。**tier が large の 9種が
「届いた%」0〜4%** で並び、その全員が `spd:0` だった:

| 魔物 | EN | 最接近(中央値) | 届いた% |
| --- | --- | --- | --- |
| 遺跡の番人 | 9 | 0px(一度も倒されず未計上) | 0% |
| 肉壁の口 | 8 | **370px** | 0% |
| 心根 | 8 | **505px** | 0% |
| 触手壺 | 7 | **489px** | 0% |
| 媚茸 | 6 | **473px** | 0% |
...
```js
/* js/game.js playCard() ——全ての魔物がここを通る */
const q=placeNear(p.x,p.y,Math.cos(a)*560,Math.sin(a)*560,24,canFly(id));
```

**彼女から 560px・無作為な方角。** 歩ける魔物はそこから寄ってくる。`spd:0` は一歩も動けない。
EN 6〜11 を払って、一度も彼女に触れないまま夜が明ける。作者の言う「出オチ」はこれ。
「遠距離チクチクの淫紋(刻印師)だけが仕事をしている」のも同じ理由で、
**あれだけが遠くから届く手**だった。

→ 三つ直した:

- **`waitSpot()`**: `spd ≦ WAIT_SPD(22)` の魔物は、彼女の**目当て**(無ければ進行方向)の先、
  `WAIT_NEAR(170)`〜`WAIT_FAR(330)`px の帯へ、`WAIT_FAN(1.5rad)` の扇で散らして置く。
  一列に並べない——どこを通っても一体は踏む。レーザー触手(spd16)・絶頂照射触手(spd20)も含む
- **ボスの湧き場所を足の速さで決める**: 一律 620px(spd>0)をやめ、`spd≧40→520 / ≧20→400 / →300`。
  spd18 の刻印師が最初の呪弾まで **34秒**かけて歩いていた
- **`BAL.BIG_GRACE`(3.2秒 / 被ダメ −55%)**: 大型・ボスは**出現直後だけ**被ダメが減る。
  硬くするのではなく「一拍目に間に合わせる」。実測で 遺跡の番人の寿命が 11秒 → 37秒

**実測(同じ16夜 / `run_cmp.js` を v7.0d と v7.1 の両方で回す)**:

| 大型・ボス | v7.0d | v7.1 |
| --- | --- | --- |
| **待ち型(spd≦22)** 最接近の中央値 | **368px** | **171px** |
| **待ち型** 仕事をした個体 | **9%** | **18%** |
| 歩き型 最接近の中央値 | 413px | 279px |

★併せて、**呪弾・光弾を下手人の帳面に載せた**(`BAL.CULP_RUNE`)。刻印師と遺跡の番人は
「触れずに効かせる」ので掴みにもダメージにも現れず、**彼女を紋だらけにして倒した張本人が
敗北文の相手に一度も選ばれなかった**。v7.0 で光線と催眠は載せたのに、紋だけ漏れていた。

#### (1) 本文の言葉 — 直すのは `pin` の拍であって、`capture` ではなかった

在庫を数えた。`capture`(結果画面の長文)は既に**動き・喘ぎ・擬音が入っている**——
v6.0 期に書いたもので、プロファイル第三部の6要素を満たしている。
読みにくかったのは **`pin` の拍**、それも **v1.x 期に書いた最初のブロック**だけだった:

```
現在(worm / ルミナ)        手首の蟲が脈に合わせて蠕動する。どちらの鼓動か分からなくなる。
                            蟲の体温が自分と同じになっていく。——境目が、溶ける。
```

行為が抽象へ逃げていて、声が三拍に一度しか無い。→ **六拍の型**に組み直した:

| 拍 | 中身 |
| --- | --- |
| 1 | **擬音つきの動き**。何が・どこを・どう掴んだか。平たい言葉で |
| 2 | **声(low)**。堪えている。子音を噛み殺す |
| 3 | **深まる動き**。部位を言い切る(内腿・乳首・秘裂) |
| 4 | **声(mid)**。否定語が混ざる。まだ言葉である |
| 5 | **無様な事実**。迎え腰・震え・濡れ。「無様だ」とは書かない |
| 6 | **中間音**(プロファイル 3B-4)。拒絶 → 自分で言い直す → 言い直しが強請りになっている |

6拍目は四人で形を変えた: ルミナ「はなして……ちが、う……そこ、そのまま、で……っ」/
フレイラ「離せ……っ、……いや、ゆるめ、るな……っ」/ クウ「……はなして。……ちがう。
……いま、ゆるめ、ないで」/ ヤミコ「……離しなさい。……ちがう。……ゆるめ、ないで」。

書き直したのは **ルミナ16種 + フレイラ22種**(各6拍)。あわせて、実際に四肢を取るのに
本文が無く `default` 落ちしていた **這い寄る手・骨兵・澱み手** を四人ぶん足した。

**★踏んだ罠(`check_scene_dup.py` を足した理由):** `scenes*.js` は最初のリテラル
ブロックのあとに `Object.assign(SCENES*.pin, {…})` が何本も続き、**同じ鍵は後から書いた方が
勝つ**(index.html の読み込み順に走る)。先頭ブロックへ丁寧に書き足した 8件が
`js/scenes_v20.js` の同じ鍵に上書きされて、**一度も画面に出ないまま残っていた**。
BAL のキー重複と同じ罠なので、同じように毎回数える。

**実測(`run_v71e` / `run_v71f`)**: 四人ぶんの `pin` は 48/48/46/45種・計1120拍。
箱(幅616px)からはみ出す拍 **0**、鉤括弧の不一致 **0**、`...` の表記ゆれ **0**。
四肢を取る魔物で本文が無いものは **0体**(残り13〜16体は掴まない種——ガス玉・綿毛・
覗き目玉など——で、押し倒しの主にならない)。

### 3-82. v7.2a ChatGPT 側で入った描画強化を、本当に動かす

作者が ChatGPT で足した描画(`js/visual_pack.js` 216行・`js/generated_assets.js`・
`js/generated_asset_hooks.js`・`assets/generated/` の webp 15枚)は、**測るとほぼ全部が
画面に出ていなかった**。Pages と同じ HTTP 配信でヘッドレス起動して数えた(`run_v72a/b.js`)。

| 出ていなかった原因 | 実測 |
| --- | --- |
| `js/main.js` が `'js/visual_patck.js'` を読みに行く(綴り違い)。読み込み失敗は `console.warn` で握りつぶして先へ進む | HTTP **404**、`window.Game4VisualPack` 未定義 |
| `generated_asset_hooks.js` が局所のハッシュを `H` と名付け、**画面の高さ `H` を覆い隠した**。同じ関数の `G.cam.y-H/2-p` が NaN になり、タイルのループが一周もしない | 戦闘129フレームで生成画像の描画 **0枚**(15枚読み込んで未使用) |
| VFX 割当 11種のうち6種(slime/splash/curse/darkburst/nova/burst)は、ゲームが一度も出さない fx 名 | — |
| beam(絶頂照射触手)・flash(ゲイザー)・denbeam(巣窟)・chain は**線や扇の形**なのに、中心へ正方形の絵を貼っていた。beam には聖なる斬撃、bolt/beam/denbeam には聖なる星 | 魔物側の光条に聖なる絵 |

直して描かせると、今度は**重さ**が出た。三層とも `drawTiles` を包んで、**毎フレーム・画面内の
全タイル**に `drawImage` と放射グラデを作り直す作りだった:

| 階 | v7.1a(追加なし) | 毎フレーム描画 | **v7.2a(焼き込み)** |
| --- | --- | --- | --- |
...
(描画1回の ms、中央値 / p90。同じ夜・同じ乱数)

この地図は元々 `renderChunk()` で **256px 四方のチャンクに一度だけ焼いて**、`drawTiles()` は
それを貼るだけの作り。重ね絵はどれもタイル位置のハッシュで決まる静止画なので、
**チャンクを焼く時に一度だけ描けばよい**。`js/map.js` に差し込み口 `CHUNK_DECOR` を作り、
三層をそこへ登録し直した(関数は `(g, x0,y0,x1,y1, i0,j0,i1,j1)` を受け、`g` はワールド座標。
一つが投げても地図は描き続ける)。地形が変わったチャンクは焼き直されるので、沼の広がり・
乾き・凍りにもそのまま追従する。魔物ごとのハイライトは原点で半径ごとに一度だけ作って
`translate` で使い回し、全画面の周辺減光と色味は寸法が変わった時だけ作り直す。
`drawImage` の回数は 47.9万 → **6.6万**(元 5.9万)、描かれた生成画像は 0 → 約7,800。

★**`js/quality_patch.js` の `aiDecide` の包み(退避・間合い取り)と、フレイラの合流深度を
5 へ上げる一行はゲーム性に効く。** v7.2 では手を入れていない(作者が ChatGPT に頼んだ中身)。

### 3-83. v7.2b 「yield を最長に」——堕ちの二軸を夜ごとの伸びで積む

カタログ第四章:

> **肉体(body)**: 比較的早く進む。段階 不惑(0)→綻び(25)→馴染み(50)→熟れ(72)→崩れ(90)
> **精神(mind)**: **肉体の1/3以下の速度**で進む
> 反応段階 `mind≥62→crave / body≥50→yield / body≥25→strain / else resist`
> **yield=「心は拒み、体は応える」(最長に滞在させる)**。
> 設計の芯: 「心は拒んでいるのに体だけが応えてしまう」期間が最も長く続くこと。**簡単には堕ちない**。

BRIEFING の既知の課題に「帯の滞在時間がプロファイルの逆」と一行だけあり、測り方が残っていなかった。
段(`fallTier()`)は夜の終わり(`endBattle`)にしか動かないので、**世代の中で何夜目にどの段にいたか**を
数える(`run_v72e.js`・120日を夜をまたいで通し・オート指揮・同じ乱数)。

**旧(v7.2a まで)**: `body = 被ダメ/8 + 状態異常×2`、`mind = 捕獲×22 + 状態異常×0.5` を、
世代内の生の累計から毎回計算していた。

| | 抵抗 | 綻び | yield | **待ってしまう** |
| --- | --- | --- | --- | --- |
| ルミナ | 17% | 12% | 36% | **36%**(連続平均4.3夜) |
| フレイラ | 11% | 9% | 32% | **47%**(5.9夜) |
| クウ | 24% | 24% | 20% | **33%**(6.0夜) |

**どの子も crave が最長か同率一位**——カタログの逆。経過を見ると理由が丸見えで、
30日目に 0/0 だったフレイラが**一晩で体100・心100**になっている。`awardAil` は種類ごとに
2秒に1回数えるので、拘束され続ける長い夜は一晩で状態異常が150近く溜まり、両軸とも天井に張りつく。
綻び(25〜50)は一夜で跨ぎ越され、捕獲の夜は心が体の半分近く伸びて「1/3以下」も破っていた。

**新(v7.2b)**: 帳簿に `R.body` / `R.mind` を持たせ、`endBattle` から一夜ぶんを `fallNight()` で積む。

- **body**: 一夜の伸びを `FALL_BODY_NIGHT`(=25 = 一段の幅)で頭打ち。重い夜はちょうど一段、
  中くらいの夜は生の値そのまま。**一夜で段を二つ跨がない**
- **mind**: 一夜の伸びを「その夜の body の伸び × `FALL_MIND_RATIO`(=1/3)」で頭打ち。カタログの文言そのもの
- 生の伸び(被ダメ/8・状態異常×2 / 捕獲22・状態異常×0.5)は旧のまま。変えたのは**一夜の上限だけ**
- 旧セーブは、最初に帳簿を開いた時に生の累計から一度だけ `body`/`mind` を起こす(途中の世代がいきなり戻らない)

| | 抵抗 | 綻び | **yield** | 待ってしまう |
| --- | --- | --- | --- | --- |
| ルミナ | 17% | 29% | **54%**(連続5.0夜) | 0% |
| フレイラ | 11% | 18% | **69%**(連続6.2夜) | 2% |
| クウ | 24% | 31% | **45%**(連続6.3夜) | 0% |
| ヤミコ | 33% | 17% | **50%**(連続6.0夜) | 0% |

**yield が四人とも最長**(夜数でも連続滞在でも)。0→25(綻び)→50(yield)と一夜に一段ずつ進み、
綻びが飛ばされなくなった(ルミナ 12%→29%)。

★途中で一度、指数で飽和させる式(`24×(1−e^(−raw/24))`)も試した。重い夜は良いが**中くらいの夜を
削りすぎ**、接触を避けて夜が軽いクウだけ綻びが yield を夜数で上回った(38% 対 33%)。頭打ちの上限に替えた。

★**crave は稀になった(0〜2%)**。心は一夜 8.3 までしか伸びないので、62 に届くには巻き戻り無しで
**8夜以上**続いた世代が要る(フレイラは40日目に心61)。到達はする——「簡単には堕ちない」の読み。
もっと見せたいなら `FALL_MIND_RATIO` を上げる(ただしカタログの「1/3以下」を越える)か、
`FALL_MIND_AIL` を上げて軽い夜でも上限に届きやすくする(1/3 の枠内で速くなる)。
