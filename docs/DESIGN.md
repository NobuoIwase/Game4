...
| id | 名 | 付与元 | 機構 |
|---|---|---|---|
| aphro | 媚薬 | ガス雲(滞在+9/s)/小淫魔の煽り(近接+2.2/s)/ナメクジ接触+4 | 0-100蓄積・減衰0.6/s。**即発情はしない**。ゲージ比例で集中低下・微減速 |
| heat | 発情 | 媚薬100到達で状態遷移 | 10秒間: 集中-35%・速度-12%・スタミナ回復ほぼ停止・もがき×0.7・ふらつき。終了後ゲージ35に戻る |
| slow | 粘液 | スライムの跡 | 速度55% |
| charm | 魅了 | ナメクジ接触(個体CD6s) | 対象を攻撃・回避の対象から除外 |
...
- `js/scenes.js` — `SCENES.pin[<id>]={beats:[]}`(押し倒し中に画面下へ順次表示)、
  `SCENES.capture[<id>]={title,beats:[]}`(敗北リザルト)。未定義時は機構的表示のみ(現状すべて未定義)
