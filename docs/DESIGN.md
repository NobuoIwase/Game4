...
- `separateEnemies`(enemiesUpdate の末尾で2回): 48pxの空間ハッシュで近い組だけ見て、`want=(r1+r2)×0.82` 未満なら重なりを質量比(r²、ボス12、設置物∞)で
  押し戻す。潜伏中・四肢に付いた個体は除く。`heroSeparate`: 彼女の中心から `(e.r+p.r)×0.62` 未満には入れない(接触判定 r+r の内側なので接触は成立)。
