...
### 3-9. v1.3 描き込み(グラフィック)

- ルミナ: `assets/sprites/lumina_hd.png`(70×105、`tools/pixelate.py --mode smooth --outline`)。`META.settings.gfx==='hd'` のとき
  `drawGirlSprite` が約60px高・smoothing ON で描き、足元の落ち影と `source-atop` の斜めグラデーション(左上の光・右下の陰)を重ねる。
  'pixel' で従来の35×52ドット版。ホーム画面で切り替え。
- 魔物: `drawEnemy` → `drawBody`(種族ディスパッチ)を `drawEnemyCached` で包む。オフスクリーンに本体を描き、
  `source-atop` で陰影(左上→右下)とハイライト、シルエットを暗色化して四方1pxずらした縁取りを敷く(`renderShaded`)。
  結果は `SPR_CACHE`(種族×半径×状態キー×アニメ位相16コマ、上限900)に保存し、以後は drawImage 1回。
  状態キーは `spriteKey` に列挙(ワームの跳躍、花の開閉、小淫魔の向き、目玉/ゲイザーの視線8方位、照射の照準など)。
  ボスはキャッシュせず毎フレーム合成(1体)。画面外の個体は描かない。ベンチ(260体・全種)で従来と同等の描画時間。
