...
const BAL={
  ...
  LANTERN_WANT:1.8, LANTERN_HEAT_MAX:34, LANTERN_CD:18, DARK_FAR_SEE:900,   /* v6.6 0.42 では清水(2.6)や宝箱に一度も勝てず、彼女は「灯篭へ行こう」と決めたことが無かった。
     実測では寄った 1.3回/夜 は全部が通りすがりの外周(k≒0.12)で、腰を下ろす 44px まで入っていない。暗い時の休み場として、清水より少し下の重みにする */
  /* v6.6 灯篭は時間ではなく「溜まった熱」で切り上げる。実測(第5層8夜)で LANTERN_STAY=3.4 の機械的な打ち切りと
     CD52秒のせいで、一晩の発情の 1.2%・敏感化の 0.3% しか作っていなかった(寄るのは1.3回・7.5秒/夜)。
     初めては気づくのが遅く、たっぷり温まる。何度か焼かれて learnTrap('lantern') が積もると早く気づき、
     LANTERN_KNOW_OFF 回で「あれは近づかない方がいい」と寄らなくなる。★世代が巻き戻ると学習は消える */
  LANTERN_NOTICE:12, LANTERN_LEAVE:26, LANTERN_KNOW_CUT:0.28, LANTERN_KNOW_OFF:3, LANTERN_REST:16,
  LANTERN_NEED_LIGHT:0.45,            /* v6.6 この階で集めた灯りがこれ未満のうちは、休み場として寄る気になる(0.16 では燭台を一つ拾った時点で用が無くなり、断りの26%がこれだった) */
  /* --- v4.0 魔核戦の専念 --- */
  ...
  /* ★v6.8 1.15 では「掴まれた」が届いていなかった。distressOf は肢一本 0.55・
     二本で 1.10 で、押し倒し(1.7)まで待たないと誰も来ない。実測でも、
     仲間が掴まれている時間のうち誰かが助けに動いていたのは 31.6% だけで、
     残りはジェム拾い(14.5%)・逃げ(12.5%)・引き撃ち(8.8%)だった。
     肢一本 + 戦いの音(FIGHT_W)で動く高さに下げる */
  COVER_TH:0.60,           // この値を超えたら「まずい」。放って探索を続けない
  /* v6.8 戦いの音。仲間が戦っている・掴まれていると音が立ち、壁越しでも届く。
     ★作者の言葉「戦闘が発生したら音を聞いたり見たりで救出に向かう」 */
  HEAR_R:820,              // 音が届く距離(壁を通る)。これより遠い戦いには気づかない
  FIGHT_W:0.55,            // 「戦っている」ぶんの重み(掴まれていなくても駆けつける理由になる)
  FIGHT_N:2,               // この数の魔物が 230px 内に居れば戦っている
  FIGHT_HURT:2.5,          // 直前に殴られてから、この秒数は戦いの音が続く
  ASSIST_HARD:1.20,        // これを超える窮地なら、逃げ・引き撃ちより救援を先にする
  VIEW_KEEP:56,            // 画面の縁に残す余白(手綱の箱 = 半画面 - これ)
  ...
  /* v6.9 宝箱。★22px の決め打ちだと、彼女の半径(14前後)のぶん中心を踏まないと開かなかった */
  CHEST_TAKE_R:26,         // 箱の肩に触れれば開く(実効は +本人の半径の半分)
  CHEST_GRAB_R:90,         // これより近い箱は、目当てが別でも道すがら寄る
  CHEST_GRAB_K:0.55,       // 歩く向きに混ぜる割合(往復にならない程度)
  DRY_EVAP_R:560, DRY_EVAP_N:6, DRY_EVAP_RATE:1.75, DRY_EVAP_LIFE:15,   // 巣窟で焼いた時: 蒸発した媚薬が外まで広がる
  /* ★v6.9 蒸発は「一度の噴き出し」で終わっていた(作者の報告
     「たまにえちえちエリアで発動したりした後、そのまま移動しても霧が強くならない」)。
     炎のエリアが褥・澱みに掛かっている間は煮え続ける形にする。
     作者の指定「罰としてもっと凶悪にして欲しい」 */
  DRY_BOIL_CD:1.1,         // 煮え続けている間、この間隔で霧を噴く
  DRY_BOIL_R:150,          // 一つぶんの霧の半径
  DRY_BOIL_LIFE:9,         // その霧が残る秒数
  DRY_BOIL_RATE:2.4,       // 霧の濃さ(SENSIT_GAS 倍)。★通常のガスより濃い
  DRY_BOIL_HEAT:22,        // 煮えている円の中に居る間の発情 /s
  DRY_BOIL_STACK:1.35,     // 煮え続けるほど濃くなる倍率の上限へ向かう係数
  DRY_BOIL_T:16,           // 煮えている秒数。★彼女が炎を落としても止まらない(罰)
  DRY_BOIL_SPREAD:230,     // 煮えた霧が湧く範囲(彼女が歩けば、その先が煮える)
  ...
  /* v6.3 媚薬沼の中の物を、目当てとしてどれだけ割り引くか。
     ★0.30(深い沼)は厳しすぎて、宝箱でも一瞬しか目当てにならず、
  ...
  /* ★v6.4b 蒸発した媚薬を吸い込む一連。前は噴き出した瞬間に効きが全部入り、
     四分の一の確率でその場で「覚えた。褥では焼かない」と言って終わっていた。
     順序を作る: うわっ(EVAP_GASP_T よろける) → 吸い込んでむせる(EVAP_CHOKE_T)
     → 霧の外へ足が向く → 出てから「覚えた」。
     効きは EVAP_NOW ぶんだけ即入り、残りはむせている間にじわじわ */
  EVAP_CHOKE_T:2.8, EVAP_GASP_T:0.7, EVAP_NOW:0.35, EVAP_CHOKE_SPD:0.6,
  /* ★v6.5 敗北のあとの観測。捕まえてすぐ結果へ行かず、形だけの抵抗を続けさせる。
     次々に押し倒され、押し倒し本文がプレイ中に読める——この作品でいちばん
     たくさん書かれていて、いちばん目に触れていなかったものがここ
     (実測: 30夜で 3058行中92行=3%。押し倒しが 0.5回/晩 しか起きないため)。
     ★ゲームは進行しない。オーブもエッセンスも経験値も、一切増えない。
     終わりはプレイヤーが決める(戦闘バーの『観測を終える』)。
     四人ぶんを同時には読めないので、主を順に回して読ませる */
  /* v6.6e 捕まってその場に残された子。★ここまでは「離脱=何も起きない」だった——
     実測(捕獲後54秒): 四肢拘束 0.0% / 責め手 0.0% / 快感が上がった瞬間 0.0% /
     倒れた場所から動いた 0.0%。半径52に魔物は 50.1% 居るのに、ただ立っていた。
     狙いは「動いて振りほどいたりはするけど、責め手が変わるだけで、されつづけている」 */
  CAP_HOLD:2,          /* 常に絡んでいる手の本数 */
  CAP_CALL_R:280,      /* この距離までの手が、捕まった子へ寄ってくる */
  CAP_GRAB_R:44,       /* v7.0 寄ってきた手が実際に絡む距離。★寄せる判定より先に見る(下の注) */
  CAP_SWAP_T:6.5,      /* 責め手が入れ替わる間隔(秒) */
  CAP_SHAKE_T:12,      /* 身を捩って一本ほどくまでの間隔(秒) */
  CAP_SHAKE_D:24,      /* ほどいて、這って逃げる距離(px)。すぐまた取られる */
  CAP_CRAWL_T:0.55,    /* v7.1 その距離を這うのにかける秒数。瞬間移動だと動いたことが見えない */
  CAP_PULSE:1.6,       /* 責めの拍(秒) */
  CAP_PLE:4.0,         /* 一拍ぶんの快感。★7.0 では絶頂している時間が 51% になった——
                          この作品の芯は「まだ抗っている心が軋んでいく過程」なので、
                          達しっぱなしにはしない(狙いは3割前後) */
  CAP_BUB_T:5.0,       /* 声の間隔(秒) */
  CAP_SAVE_R:900,      /* この距離までなら、捕まった子を助けに行く */
  CAP_SAVE_HP:0.35,    /* 体力がこれを割ったら、捕まった子より自分の命 */
  CAP_SAVE_THREAT:2.2, /* この脅威までは踏み込む。★救出は「目当て」だったが、
                          目当ての枝は threat>0.9 の逃げ・引き撃ち・直接目標より下にあり、
                          仲間が捕まった後は脅威が濃いので一度も見られていなかった
                          (実測: 相方が救出へ向かう 0.0% / 救出 0回)。
                          かばい(assist)と同じ段に、独立した枝として置く */
  AFTER_ON:1, AFTER_FIRST:2.8, AFTER_PIN_T:26, AFTER_GAP:1.7,   /* PIN_T は保険の上限。ふつうは本文を読み切った時点で次へ移る */
                       /* v7.0 一律の送り AFTER_BEAT_T は廃止。行の長さで決める(PIN_BEAT_BASE/CPS) */
  AFTER_CALL_R:420,    /* v7.0 観測: 主が一本も取られていない時、この距離までの手を呼んで取らせる */
  AFTER_STRUG:7.0,     /* v7.1 観測中のもがきの溜まり(毎秒)。快感・発情で落ちる */
  /* v7.1 待ち型(spd≦WAIT_SPD)の置き場所。彼女の目当ての先、この帯に扇で散らす */
  WAIT_NEAR:170,       /* 近い方の縁(px)。これより近いと「湧いた瞬間に掘まる」になる */
  WAIT_FAR:330,        /* 遠い方の縁(px)。彼女がこれから歩く距離 */
  WAIT_FAN:1.5,        /* 扇の広さ(rad)。一列に並べない——どこを通っても一体は踏む */
  ...
  CULP_SRC_T:8.0,      /* 同: 光線・催眠の源を「まだ効いている」とみなす秒数 */
  CULP_BEAM:7.0,       /* 同: 照射を浴びている 1秒ぶんの重み(掴む手の CULP_HOLD に近い取り分) */
  CULP_HYPNO:4.0,      /* 同: 催眠が掛かっている 1秒ぶんの重み(Lv倍) */
  CULP_GAS:5.0,        /* 同: その魔物が撒いた雲の中に居る 1秒ぶんの重み(濃さに比例) */
  PIN_BEAT_BASE:0.8,   /* v7.0 本文の送り: 一行あたりの最低の間(秒) */
  PIN_BEAT_CPS:11,     /* 同: 一秒あたり何字ぶん置くか。26字なら 0.8+2.4=3.2秒(毎分約490字) */
  /* ★v6.3b 媚薬のベタベタ。沼から出ても身体に残り、洗うまで火照りが上がり続ける。
     「沼に浸かった」が、その場だけの出来事で終わらないようにする */
  STICKY_GAIN:0.55,    /* 沼の中で1秒あたり溜まる濃さ(深さで倍) */
  STICKY_MAX:3.0,      /* 濃さの上限 */
  STICKY_HEAT:2.1,     /* 濃さ1あたり、毎秒の火照り */
  STICKY_SENS:0.5,     /* 濃さ1あたり、毎秒の敏感化 */
  STICKY_DRY:0.012,    /* 何もしないと、ごくゆっくりしか薄れない(乾くだけで落ちはしない) */
  STICKY_WADE:0.34,    /* 浅瀬を歩くと、この速さで流れる */
  STICKY_WIPE:0.9,     /* 手で拭う(足が止まる)一回で落ちる濃さ */
  STICKY_WIPE_T:1.4,   /* 拭うのに掛かる秒 */
  STICKY_WIPE_TH:1.2,  /* この濃さを越えると、拭おうとする */
  STICKY_WIPE_CD:7,    /* 拭い直すまでの間 */
  ...
  /* v6.6g 小淫魔は現場指揮官。部下が居なければ意味がない。
     ★実測: コスト8・HP20 の一体を死ぬたび出し直していて、一晩のENの 13.5% を
       独りで食っていた(10回で120EN。ゴブリンは65回で130EN)。
       雑魚が揃っている時だけ、間を置いて呼ぶ */
  IMP_NEED_FODDER:7, IMP_RECALL:18,
  ICE_CD:7, ICE_MAX:2400, ICE_WANT:2.2,   // 引き直しの間隔 / 一つの階層に残る氷タイルの上限 / 方角の点数がこれを超えたら引く
  /* v6.6f 自分から橋を架ける: 目当てまでの直線のうち、これだけが沼・浅瀬なら、
     敵が居なくても凍らせて道にする。★氷の上は沼に浸かっていない扱い(miresTick の onIce)
     なので、渡れるようになるという効きは既にある——足りなかったのは「引く判断」だけだった */
  ICE_BRIDGE_WET:0.34, ICE_BRIDGE_MIN:150,
  ...
  CROSS_FAN_PX:13,     /* v6.3 十字が複数飛ぶ時の、的の位置での横のずれ */
  ...
  DEN_SEED_LEASH:110,  /* v6.3 巣窟の住人が、輪の外へ出てよい距離 */
  /* ★v6.3b 効きを二つに割る。作者の基準は
     「入った瞬間に殴られるのは困る / でも第一層でも出る頃には発情1になっていてほしい」。
     一つの倍率では両立しないので、【一撃】と【積もり】を別々に刻む。
     実測(1階の巣窟を歩いて通らせる): 一本の倍率 0.42 では、出た時 火照り81/100・発情Lv0 だった */
  /* v6.7 居た場所で数え直すと、巣窟は「時間の 6.3% で発情の 30.5%」——
     毎秒 25.1 対 外 3.9 で 6.4倍。★以前の「巣窟の仕掛けは発情の 0.1%」は
     denTick が湧かせた雲だけを数えた狭い見方で、床の手・陣・沼・番人・奥の一撃が
     抜け落ちていた。
     ★v6.8 それでも数字は元に戻した。20%薄くして48日回したところ、
       勝敗はまったく動かず(降下 63%→71%→67% は同じ幅の揺れ)、
       一日の絶頂だけが 3.2回 → 1.6回 に半減した。純粋な損。
       「入って負ける」の実体は中の濃さではなく、外へ持ち出す状態のほうだった */
  DEN_POW0:0.58,       /* 積もり(毎秒の火照り/敏感化・床の手・陣): 1階でも通れば効く */
  DEN_POW_K:0.048,     /* 一階ごとに足す分。15階で 1.25 */
  DEN_POW_MAX:1.28,
  /* ★v6.3b 最奥へ踏み込んだ瞬間の一撃。敷居の一撃とは別で、こちらは浅い階でも重い。
     ★毎秒の積もりだけでは第一層の発情1に届かない——実測で、彼女が巣窟の中に
     居るのは 8〜10秒しかなく、その秒数では どんな毎秒の値でもゲージ100に届かないため。
     「奥に踏み込んだ」という出来事そのものを一撃にすると、短い滞在でも必ず刻める */
  DEN_DEEP_HEAT:46, DEN_DEEP_SENS:20,
  /* ★最奥の一撃は「どの階でも最低これだけは刻む」という床なので、深さで増やさない。
     増やすと深い階で積もりと二重に効いて、一度入っただけで発情3(上限)に達した
     (実測: 5階 3.0 / 8階 2.9、しかも 8階では 8回中5回 出てこられなかった) */
  DEN_DEEP0:0.80,      /* どの階でも 0.80 */
  DEN_DEEP_K:0,
  DEN_BURST0:0.42,     /* 一撃(敷居をまたいだ瞬間・番人の太さ): 浅い階では軽い */
  DEN_BURST_K:0.057,   /* 15階で 1.22 */
  ...
  /* --- スタミナ / 抵抗 / 押し倒し --- */
  /* ★v6.6f 作者の体感「スタミナがすごく低い」。実測すると平均は 80〜88%、中央値は 96〜100% で、
     数字の上では低くない——低いのは「戻り」だった。発情中の戻りが 0.4/s しか無いのに、
     引き剥がし1回が 14。一度取られると、返すのに 35 秒立ち尽くす計算になる。
     戦いの最中はほぼ発情しているので、体感は「一度削れたら戻らない」になる。
     最大値も少し上げ、戻りを引き上げた(特に発情中) */
  STAMINA_MAX:115,         // v6.6f 100→115
  STAMINA_REGEN:2.6,       // 非拘束時 /s   // v5.7 3.5→2.0 / v6.6f 2.0→2.6
  STAMINA_REGEN_HEAT:1.1,  // 発情中 /s   // v5.7 1→0.4 / v6.6f 0.4→1.1(ここが体感の芯)
  ...
  /* ★v6.8 4.0 / 6.0 では強すぎた。48日を二度回して、勝敗は誤差の幅でしか動かないのに
     一日の絶頂だけが 3.2回 → 1.3回 →(帯を切っても) 0.5回 まで落ちた。
     この作品でヒロインが「もういいや」と巣窟へ入る枝は火照りで開くので、
     火照りを冷ますと、えちえちエリアへ行く回数まで一緒に落ちる。
     直したいのは 91秒引きずることだけなので、1/2 以下に落とす */
  CALM_SENS:1.8,           // 上乗せされる敏感化の抜け /s(合計 1.1→2.9)
  CALM_HEAT:2.5,           // 上乗せされる発情ゲージの抜け /s(合計 1.5→4.0)
  /* ★上乗せが効くのは「振り切れた上の帯」だけ。下地はいつもの 1.1 / 1.5 で抜ける。
     48日のA/Bで、帯を区切らないと絶頂が 3.2回/日 → 1.3回/日 まで落ち、
     巣窟の奥へ入る回数も 90 → 54 になった(火照らないので「もういいや」が出ない)。
     直したいのは「91秒引きずること」だけで、火照りそのものではない */
  CALM_SENS_FLOOR:35,      // 敏感化がこの値より上の間だけ、速い抜けが効く
  CALM_HEAT_FLOOR:40,      // 発情ゲージも同じ
  ...
  BEAM_AIM:1.0, BEAM_CD:9, BEAM_LEN:300, BEAM_W:14,   /* 絶頂照射: 照準1秒→発射。彼女が見て判断できる速さ(学習で避ける)。巣窟の壁の光線も同じ照準を使う
                                        ★v6.6f 「一瞬だけ絶頂させるビーム」と「持続する快感のレーザー」は別の魔物に戻した(§3-66) */
  ...
  CAP_FIELD_N:5,                                     /* v6.6b 床から生える咳き茸の同時上限(菌輪の縁を優先) */
  ADDICT_MAX:5, ADDICT_DECAY:0.009, ADDICT_SEEK:0.6,
  /* ★閾値を 1 にしていた版は、中毒がちょうど 1.0 の夜に一秒も成立しなかった——
     +1 で 1.0 になり、次のフレームから減るので `>=1` が一瞬も真にならない。
     実測で、二度キメた夜(2.0)は 161.7秒 成立し、一度だけの夜は 0.0秒。閾値は値より下に置く */
  ADDICT_R:760, ADDICT_WORTH:2.2,                                  /* 中毒で罠に寄る時の目当ての重み */
  ...
  /* ★v6.9 条は「動かない線」になった(作者の指定)。地形として読ませたいので、
     出ている時間を伸ばし、消えている時間を詰める——通り道が塞がっていて、
     しばらくしたら別の向きに開く、という見え方にする */
  RAY_ON:4.2, RAY_OFF:2.6,/* 出ている時間 / 消えている時間 */
  RAY_PLE:16,             /* 条に浴びている間の快感(毎秒)。★浴び続けても「溜まるだけ」 */
  /* ★v6.9 RAY_TURN(条が向きを変える速さ)は削除した。
     条は 8方位のどれかへ固定して出るので、追尾そのものが無くなった */
  RAY_AIM_AVOID:0.55,     /* 彼女が居る向きを選びにくくする分(狙い撃ちにしない) */
  RAY_DEEP:9,             /* この深さから、条が絶頂まで持っていく個体になる */
  BEAM_WAKE:2.4,          /* ★湧いてからこの秒数は条を出さない(出た瞬間の事故を潰す) */
  /* v6.6f 条は避けるもの。線からの距離がこの幅に入ったら、横へ抜ける向きを足す。
     溜め(warm)の間から動き出すので、点く頃には軸から外れている */
  RAY_AVOID_W:46, RAY_AVOID_WARM:0.9, RAY_AVOID_ON:1.7,
  RAY_SRC_T:1.4,          /* これだけ浴びたら「元を断つ」に切り替える(秒) */
  RAY_SRC_GAIN:2.2,       /* 浴びている間の溜まり方。★思考の拍は毎フレームではないので、
                             素の dt で足すと 1.4 に届く前に減衰に食われる(実測 0.0%) */
  RAY_SRC_R:420,          /* 条の元へ向かう距離の上限 */
  ...
  PARTY_LEASH:620, PARTY_MAXDX:740, PARTY_MAXDY:360, PARTY_SEP:96, PARTY_SEP_K:0.85, RESCUE_R:60, RESCUE_T:3.0, RESCUE_GRAB_K:0.5, TALK_T:1.3, ASSIST_R:120, HEART_YIELD:0.15, PARTY_HOLD:7, PARTY_TALK_CD:5,   /* v6.8 10→5。決め直しの 37.5% がこの間隔で潰れていた */   // v5.0 くっつきすぎない: 寄る力は 620px から / 96px より近いと離れる
  ...
  /* v6.8 「歩きながら近寄る」ほうの門番。立ち止まる方(GATHER_DANGER_*)より緩い——
     小物が一匹視界の隅に居るくらいで相談が止まっては、洞窟ではまず成立しない */
  GATHER_BUSY_R:96, GATHER_BUSY_THREAT:1.0,
  /* ★v6.9 水鏡の女王を「討伐可能」にする三つ目の道。
     乾燥(フレイラ)・氷河(クウの進化)だけでは、二人が居ない周回で物理的に本体へ届かなかった。
     映り身を払い切れば、映す面そのものが保てなくなる。
     ★この四つを最初 MONSTERS の中に書いてしまい、MONSTERS の走査(m.deep=d)が
       「数値にプロパティを付けた」で落ちて data.js が途中で死んでいた。BAL に置くこと */
  MIRROR_SHARD:6,          /* 世代1で必要な映り身の数 */
  MIRROR_SHARD_K:1.5,      /* 世代ごとに増える分 */
  MIRROR_SHARD_MAX:16,     /* 上限 */
  MIRROR_EXPOSE_T:9.0,     /* 割れて本体が出ている秒数(この間に削る) */
  /* ================= v6.9 巣窟の支度 =================
     作者の指定「えちえちエリアに無策で突っ込みすぎる」「クウがいるなら道を凍らせて通る等、
     攻略に対するヒロインの思考が欲しい」。境での選択肢を三つにする——
     入る / 諦める / ★支度してから入る */
  DEN_PREP_WET:0.30,       // 境までの道がこれだけ濡れていれば、クウが先に凍らせる
  DEN_PREP_STAM:0.55,      // 誰かのスタミナがこれ未満なら、外で息を整えてから
  DEN_PREP_HEAT:2,         // 誰かの発情Lvがこれ以上なら、冷ましてから
  DEN_PREP_T:9.0,          // 一つの支度に掛ける上限(秒)
  DEN_PREP_STEP:0.8,       // 支度中、ひと呼吸ぶん迷いを延ばす間隔
  DEN_PREP_CD:6.0,         // 支度が済んだ(or 諦めた)あと、次の支度までの間隔
  /* ★確率の抽選にすら掛けず、その場で諦める線。
     いままでは aroused が +0.25 の加点だったので、火照っているほど入りやすかった
     ——「無策で突っ込む」の最後の一枚がこれ */
  DEN_VETO_STAM:0.38,      // スタミナがこれ未満なら、褥には入らない
  DEN_VETO_HEAT:3,         // 発情Lvがこれ以上なら、褥には入らない
  /* ================= v6.0 新しい地形の効き =================
  ...
  TRAIT_FADE:0.45,
  /* v6.3 境で迷う長さ。★よく知っている所ほど短い(見た瞬間に決まる)。
     前は逆で、覚えた巣窟の前がいちばん長かった(2.0〜3.6秒) */
  HESIT_GO_BASE:0.68,  /* v6.3 迷った末に「入る」側へ倒れる素の目 */
  HESIT_GO_KNOW:0.42,  /* v6.3 覚えているほど入らない(zoneKnow 0〜1 で引く) */
  HESIT_KNOWN:0.9,     /* 嫌い方3 = 覚えた地形。入るにせよ諦めるにせよ、迷わない */
  HESIT_NEW:1.7,       /* 嫌い方2 = まだ半端。ここがいちばん人間くさく迷う */
  ...
  /* v6.3 据わった個体に掴まれている間、彼女が繋ぎを越えて歩こうとした時に引き戻す速さ。
     ヒロインの足(SPD≒170)より速くないと、根の張った番兵を引きずって歩いてしまう */
  SEAT_PULL:280,
  SEAT_SNAP:150,       /* 壁を挟んで引き戻せない距離まで離れたら、繋ぎの方が切れる */
...
const MONSTERS={
  ...
  beamer:{
    ...
    desc:'先端に水晶の眼を持つ細長い触手。照準線が彼女に触れてから発射までが速く、細い光条に当たった者は身体の準備を待たずに達してしまう。撃った後はしばらく眼を閉じる——避けるなら、その一秒を見ていること。',
    trait:'照準1.0秒(最後の0.25秒は固定)→細い光条(射程300)。命中で【強制絶頂】。CD9秒',
  },
  /* v6.6f 「持続する快感のレーザー」。★絶頂照射触手(一瞬で達させる burst)とは別の魔物。
     v6.6 で両者を一体に混ぜてしまっていたので、分け直した */
  raytent:{
    name:'レーザー触手', role:'持続照射', cost:9, unlock:660, tier:'large',
    hp:135, spd:16, r:13, dmg:0, xp:11,
    desc:'胴から三方へ、細い光の条を放ったまま動かない触手。狙ってはいない——縦か横か斜めか、その時の気分の向きへ、壁に当たるまで一直線に伸ばして、そこに置いておく。追ってこないので当たりはしない。ただ、通りたい道の上に在る。条が消えるのを待つか、遠回りするか、元を断つか——彼女はいつもその三つから選ばされる。',
    trait:'三方向へ、8方位のどれかへ固定した光条(壁まで)。★条は動かない——追尾しない代わりに、4.2秒そこを塞いで2.6秒消える。浴びている間ずっと快感が積もる。★第9層より深い個体は達しても照らし続け、連続絶頂になる(仲間が遮るか、条の元を断つしかない)',
  },
  bossgazer:{
...
Object.assign(MONSTERS, {
  ...
  mirrorqueen:{
    ...
    desc:'湖の中央に据わり、周りの水面をすべて凪がせる。以後は水面に映った姿だけを責める。本体に攻撃は通らない。倒すには映す面を壊すしかない——炎で蒸発させるか、氷で凍らせるか、あるいは映り身を払い切るか。映すものが尽きれば、面はもう保たない。頭部に顔は無く、磨かれた鏡面が、覗き込んだ側を映している。',
    trait:'水面を凪がせ、映り身を無限に湧かせる。【映す面が壊れている間だけ】本体が露出する。壊す手は三つ——炎で乾かす / 氷で凍らせる / ★映り身を規定数払う(世代で増える。払い切ると9秒だけ本体が出る)',
  },
  nevermet:{
...
const ZONES={
  ...
  lewd:     { name:'甘い褥',     col:'#7a2a5a', fear:3, innate:0.34, desc:'v3.2 巣窟の中。奥へ入るほど効きが強い(前室→沼→最奥)。入った瞬間に発情が跳ね、魔法陣・媚薬の花・壁の光線・番人が待つ。いちばん奥に王の宝箱。★v6.3 innate を 1 から 0.34 へ。1 だと嫌い方が最初から 3 に張りついて、何度やられても振る舞いが変わらなかった(実測: 知識0でも3.00・知識満でも3.00)。0.34 なら 知らない=1.02「ちょっと嫌」→軽い気持ちで入る / 二度めまで=2.00「できれば避けたい」→短く迷う / 覚えた=3.00「入りたくない」→さっさと諦める', her:'いいものがある……でも、からだがへんになる' },
...
const FLOORS=[
  ...
  { id:'f10', name:'胎の回廊',   sub:'達することが、許可制になる',            depth:10, dark:0.94, zoneW:{womb:5,nest:3,flesh:2,ruin:1,flower:1},  wall:'flesh', en:{start:2.2,base:3.2,regen:3.2,max:3.2},     hero:{hp:2.12,dmg:1.90,stam:2.18}, mon:{hp:3.2,dmg:1.75},  affinity:['nichelord','gtent','pot','hand','mouth','succuqueen','inyoku','hugcap','worm','coughcap'], col:'#ff8fb3', lewd:{name:'産みの褥', sub:'いくつもの窪みが並ぶ褥。どれも、ちょうど人の形をしている。奥に王の宝箱', mix:{rune:3, flower:2, beam:3, grip:1.7,  seed:'pot', seedN:3, trait:'niche'}, /* v6.3 窪み: 人の形の窪みが並び、そのいくつかは壺 */ guard:'mouth', beam:'climax', guardSub:'窪みの一つが裂けて、内側から口がひらく'} },
  { id:'f11', name:'渦の縁',     sub:'抗いだけが遅れて届く。快感は遅れない',  depth:11, dark:0.96, zoneW:{stall:4,ruin:3,glyph:2,flesh:2,nest:1,damp:1}, wall:'brick', en:{start:2.4,base:3.6,regen:3.6,max:3.6},     hero:{hp:2.30,dmg:2.04,stam:2.36}, mon:{hp:3.6,dmg:1.90},  affinity:['stiller','tallykeeper','gallery','echoer','glyphmite','guardian','sentinel','runemage','gazer','beamer','raytent','eye','ghost','ghosthand','tower'], col:'#b09aff', lewd:{name:'澱みの寝台', sub:'同じ夜が何度も繰り返された寝台。凹みだけが増えていく。奥に王の宝箱', mix:{rune:3, flower:2, beam:4, grip:1.2,  seed:'stiller', seedN:2, trait:'stall'}, /* v6.3 澱み: 抗いだけが遅れて届く */ guard:'guardian', beam:'hypno', guardSub:'寝台の脇の石像が、いま目を開けた'} },
  { id:'f12', name:'渦の中心',   sub:'三つの喉。選ばなければ、進めない',      depth:12, dark:0.97, zoneW:{flesh:4,nest:3,ruin:2},                  wall:'flesh', en:{start:2.6,base:4.0,regen:4.0,max:4.0},     hero:{hp:2.48,dmg:2.18,stam:2.54}, mon:{hp:4.0,dmg:2.05},  affinity:['seatflesh','frostbud','stiller','silkmite','core','gtent','mouth','pot','hand','succuqueen','gobking','runemage','raytent'], col:'#ff4d7f', lewd:{name:'巻き戻しの褥', sub:'ここで何をされても、朝には無かったことになる褥。だから際限がない。奥に王の宝箱', mix:{rune:3, flower:3, beam:5, grip:1.4,  seed:'gtent', seedN:2, trait:'beam'}, /* v6.3 光線: 壁じゅうが目になっている */ guard:'gtent', beam:'climax', guardSub:'褥の奥から、幾度も同じ形をなぞってきた触手が伸びる'} },
...
const SPECIES_MAX={gazer:4, fluff:14, coughcap:8, peeper:6, raytent:3, beamer:5};   /* v6.6f 条が格子になると避けようがない / 絶頂照射も実測で場に74体まで増えていた */   /* v6.6 実測で綿毛が場に65体まで増えた。分裂ではなく、オート指揮が安いカード(コスト2)を連打していたのが原因。カード側にも上限を置く */                                  // 同時に場に出せる上限(種族)
...
const FAM_OF={
  ...
  peeper:'eye', eye:'eye', gazer:'eye', gazerpair:'eye', gallery:'eye', beamer:'eye', beamtwin:'eye', raytent:'eye',
...
/* v6.3 据わった個体 — 根や土台がその場にあるので、掴んだまま連れ歩かれない。
   これが無いと、番兵が輪から、沼の触手が沼から引きずり出され、
   離した後も彼女の足元に立っているので「何も無い床でまた掴まれる」ことになる。
   ★足の速さ 0 の種は全部これ。上位個体は base で見る(変種は MONSTERS に後から生える) */
const SEATED_FORCE=new Set(['gtent','sentinel','miretent']);   /* 数値の上では動くが、輪や沼から離れない */
function isSeated(id){
  if(SEATED_FORCE.has(id)) return true;
  const M=(typeof MONSTERS!=='undefined')?MONSTERS[id]:null;
  if(!M) return false;
  const b=M.base||id;
  if(SEATED_FORCE.has(b)) return true;
  return ((MONSTERS[b]||M).spd|0)<=0;
}
...
/* v6.6g 置き場の狂い。★実測: オート指揮の媚薬の霧壺は、彼女からの距離の中央値が 5px
   (最小 0px)——毎回きっちり足元に置いていた。夜の側は「そこに居そうな所」しか読めない、
   という建てつけなので、置きは外れる。速く動いている相手ほど大きく外れ、
   組み伏せている相手にはよく当たる。作者の報告:「媚薬霧や淫紋の魔法陣などが的確すぎて、
   モンスターがまともに近くにいないのに発情して絶頂する」 */
const ITEM_MISS_WALK=130;   /* 走っている相手を先読みして置く時の狂い(px) */
const ITEM_MISS_STAND=84;   /* 立ち止まっている相手の足元へ置く時の狂い */
const ITEM_MISS_HELD=26;    /* 組み伏せている相手には、ほぼ当たる */
const NIGHT_ITEMS={
...
/* v6.9 deep: 「このくらいまで潜ってから来る」の目安(META.deepest と比べる)。
   ★浅い所で足踏みし続けても来ないのは別の不幸なので、
     目安は「+2回の巻き戻し」で越えられる(partyJoinCheck) */
const PARTY_JOIN=[ { id:'freila', minEra:0, resets:1, lateEra:4, deep:4 },
                   { id:'kuu',    minEra:0, resets:1, lateEra:9, deep:8 } ];   // ヤミコは PARTY_JOIN を通らない(クウ参戦→眠り手→救出→参戦の三段。yamiAdvance)
