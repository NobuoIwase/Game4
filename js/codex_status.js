...
function reviewYamiko(c, tier, sp, bodyStg, mindStg){
  ...
  if(c.runs===0){
    ...
    p.push('もう一つ。この対象は、こちらが時を巻き戻したことを覚えている。巻き戻した側は忘れる決まりだが、彼女は渦の中心で生まれたので、その決まりの外に居る。——互いが互いの思うようになるまで続くのだと、彼女は言った。言って、それでも降りてくる。');
    return p;
  }
  if(tier==='resist'){
...
function selfYamiko(c, tier, sp){
  ...
  if(c.runs===0){
    ...
    lines.push('「一つだけ。……この頁、前にも書かれているの。あなたが忘れているだけ」');
    heart='（……書式まで同じとは、思わなかった。何度目かも、書いてある）';
  }else if(tier==='resist'){
