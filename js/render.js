...
function drawHUD(g){
  ...
  /* 視界は端末の縦横比に合わせて幅が変わる。狭いときは横並びをやめて段に積む。
     右端はミュートボタン(DOM)と重なるので、そのぶん余白を取る。 */
  const narrow = W < 780;
  const RM = narrow ? 56 : 16;   // 右端はミュートボタンと重なるので広めに空ける
  // XPバー(ヒロインの成長=脅威度)
  ...
  const barW = narrow ? Math.max(120, W-130) : 150;
  ...
  g.fillStyle='#ffffff'; g.font='bold 8px '+FONT;
  g.textAlign = narrow?'right':'left';
  if(narrow){ g.shadowColor='rgba(0,0,0,0.9)'; g.shadowBlur=3; }
  ...
  g.font=(narrow?9:10)+'px '+FONT; g.fillStyle='rgba(200,180,255,0.8)';
  const genLine='第'+genNum(META.gen.idx)+'世代 / 戦歴'+(META.gen.battle+1)+'/'+BAL.GEN_LEN;
  g.fillText(genLine, narrow?W-10:W-RM, narrow?barY+18.5:42);
  // 夜の深まり(彼女のLv連動の夜側強化)
  ...
  if(boss){
    ...
    const bw=Math.min(300, W-40), by=narrow?H-72:60;
  ...
  if(p.goal && G.mode==='battle'){
    ...
    const gx0=narrow?10:10+cw+8, gy0=narrow?chipY+24:48;
...
function drawCards(g){
  ...
  g.shadowBlur=0;
  const cw=168, ch=186;
  // 縦長の画面では3枚を横に並べきれないので縦に積む
  const vert = W < cw*3+56;
  c.opts.forEach((k,i)=>{
    const chosen=c.revealed&&i===c.pick;
    const isEvo=k.startsWith('EVO:');
    const def=isEvo?EVOS[k.slice(4)]:UPG[k];
    const px=vert ? W/2-cw/2
                  : W/2+(i-1)*Math.min(196,(W-24)/3)-cw/2;
