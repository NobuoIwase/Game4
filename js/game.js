...
/* 武器レベルは Lv5 までが「型」、Lv6 以降は「熟練」。
   Lv5 で融合進化が解禁され、そこまでの効き方は今までと同じ。Lv6 からは
   どの武器にも同じ割合で乗る倍率にしてある。進化して式が固定値になった武器でも
   レベルが伸びる(＝上限に達してレベルが無駄になることがない)。 */
const EVO_LV=5;
const mastN=(h,k)=>Math.max(0,(h.wp[k]||0)-EVO_LV);
const mDmg=(h,k)=>1+0.12*mastN(h,k);           // 与ダメージ +12%/Lv
const mCd =(h,k)=>Math.pow(0.94,mastN(h,k));   // 攻撃間隔  -6%/Lv
const mR  =(h,k)=>1+0.05*mastN(h,k);           // 効果範囲  +5%/Lv
const orbN=h=>Math.min(8,h.wp.orb||0);          // 光球の数(見た目が潰れないよう8個まで)
...
function aiDecide(foc){
  ...
  if(attachCount(p)>0){
    ...
    if(!target){
      ...
      let bestGm=null, bd=walk?150:430, bestCl=null;
      ...
      for(const gm of B.gems){
        ...
        if(d>=bd) continue;
        if(walk && ((gm.x-p.x)*gx+(gm.y-p.y)*gy)/(gdn*(d||1))<-0.3) continue;   // 歩くときは進む先のジェムだけ
...
function weaponsUpdate(dt){
  ...
  if(p.wp.bolt>0){
    ...
    if(p.boltT<=0){
      ...
      const lv=Math.min(EVO_LV,p.wp.bolt), md=mDmg(p,'bolt'), mc=mCd(p,'bolt');
      ...
      if(wantProp){
        ...
        if(d<460 && B.bullets.length<150){
          ...
          B.bullets.push({x:p.x,y:p.y-14,vx:Math.cos(a)*460,vy:Math.sin(a)*460,
            dmg:(15+5*(lv-1))*md, pierce:0, life:1.2, last:null, evo:false});
          S.pew();
        ...
        if(ts.length && B.bullets.length<150){
          p.boltT=(evo?0.55:0.7)*Math.pow(0.87,lv-1)*mc;
          for(let i=0;i<shots;i++){
            ...
            B.bullets.push({x:p.x,y:p.y-14,vx:Math.cos(a)*sp,vy:Math.sin(a)*sp,
              dmg:(evo?21:15+5*(lv-1))*md, pierce:evo?2:(lv>=4?1:0), life:1.3, last:null, evo});
          }
          S.pew();
          if(restraintCount(p)>0) addStruggle(BAL.STRUGGLE_SHOT_GAIN);
  ...
  if(p.wp.orb>0){
    p.orbAng += (2.0+0.15*orbN(p))*dt;
  }
  if(p.wp.nova>0){
    p.novaT-=dt*atkMult;
    if(p.novaT<=0){
      const evo=p.evo.sburst>0;
      const lv=Math.min(EVO_LV,p.wp.nova);
      p.novaT=((evo?4.0:4.3)-0.4*(lv-1))*mCd(p,'nova');
      const R=(evo?180:100+20*(lv-1))*areaMult(p)*mR(p,'nova'), dmg=(evo?34:16+7*(lv-1))*mDmg(p,'nova');
  ...
  if(p.wp.whip>0){
    p.whipT-=dt*atkMult;
    if(p.whipT<=0){
      const evo=p.evo.srush>0, lv=Math.min(EVO_LV,p.wp.whip), mr=mR(p,'whip');
      p.whipT=(evo?0.65:1.0)*Math.pow(0.9,lv-1)*mCd(p,'whip');
      p.whipSide*=-1;
      const range=(evo?165:105+11*lv)*areaMult(p)*mr, half=(evo?165:46+5*lv)*areaMult(p)*mr;
      const dmg=(evo?22:10+4*(lv-1))*mDmg(p,'whip');
      p.whipAnim=0.16;
  ...
  if(p.wp.rain>0){
    p.rainT-=dt*atkMult;
    if(p.rainT<=0){
      const evo=p.evo.scomet>0, lv=Math.min(EVO_LV,p.wp.rain);
      p.rainT=(evo?1.5:2.3)*Math.pow(0.88,lv-1)*mCd(p,'rain');
      ...
      for(let i=0;i<drops;i++){
        ...
        if(B.bullets.length<170){
          B.bullets.push({kind:'rain', x:tx+rand(-40,40), y:ty-300, tx, ty,
            vx:0, vy:540, dmg:(evo?26:12+5*(lv-1))*mDmg(p,'rain'), splash:(evo?76:48)*areaMult(p)*mR(p,'rain'), life:1.0, last:null, evo});
          fired=true;
        }
      }
  ...
  if(p.wp.cross>0){
    p.crossT-=dt*atkMult;
    if(p.crossT<=0){
      const evo=p.evo.sjudge>0, lv=Math.min(EVO_LV,p.wp.cross);
      const ts=nearestEnemies(1,500);
      if(ts.length && B.bullets.length<170){
        p.crossT=(evo?1.3:1.7)*Math.pow(0.9,lv-1)*mCd(p,'cross');
        ...
        for(let i=0;i<nC;i++){
          ...
          B.bullets.push({kind:'cross', x:p.x, y:p.y-12, vx:Math.cos(a2)*sp, vy:Math.sin(a2)*sp,
            spd:sp, dmg:(evo?20:9+4*(lv-1))*mDmg(p,'cross'), retT:evo?0.55:0.42, ret:false, life:2.4, last:null, evo});
        }
  ...
  if(p.wp.sanct>0){
    const evo=p.evo.gsanct>0, lv=Math.min(EVO_LV,p.wp.sanct);
    p.sanctPulse+=dt*atkMult;
    p.sanctR=((evo?130:70+8*lv))*areaMult(p)*mR(p,'sanct');
    if(p.sanctPulse>=0.5){
      p.sanctPulse-=0.5;
      const dmg=(evo?14:6+3*(lv-1))*mDmg(p,'sanct');
      let hit=false;
      for(const e of B.enemies){
        if(e.dead||e.dormant) continue;
  ...
  if(p.wp.blade>0){
    p.bladeT-=dt*atkMult;
    if(p.bladeT<=0){
      const evo=p.evo.kblade>0, lv=Math.min(EVO_LV,p.wp.blade);
      p.bladeT=(evo?0.42:0.85)*Math.pow(0.9,lv-1)*mCd(p,'blade');
      ...
      for(const dir of dirs){
        for(let i=0;i<n;i++){
          ...
          B.bullets.push({kind:'blade', x:p.x+dir*8, y:p.y-14+(i-(n-1)/2)*4, vx:Math.cos(spread)*sp*dir, vy:Math.sin(spread)*sp,
            dmg:(evo?16:10+3*(lv-1))*mDmg(p,'blade'), pierce:evo?3:1, life:0.9, last:null, evo});
        }
      }
  ...
  if(p.wp.thunder>0){
    ...
    if(p.thunderT<=0){
      const evo=p.evo.judgment>0, lv=Math.min(EVO_LV,p.wp.thunder);
      const n=(evo?6:1+Math.floor((lv+1)/2))+dupN(p);
      const ts=nearestEnemies(n*3,440);
      if(ts.length){
        p.thunderT=(evo?2.0:2.6)*Math.pow(0.9,lv-1)*mCd(p,'thunder');
        const splash=(evo?52:34)*areaMult(p)*mR(p,'thunder'), dmg=(evo?30:18+6*(lv-1))*mDmg(p,'thunder');
  ...
  if(p.wp.holy>0){
    ...
    if(p.holyT<=0){
      ...
      const evo=p.evo.spring>0, lv=Math.min(EVO_LV,p.wp.holy);
      const n=(evo?3:1+Math.floor((lv-1)/2))+dupN(p);
      const ts=evo?nearestEnemies(n*2,420):[];
      if(!evo || ts.length){
        p.holyT=(evo?2.4:3.0)*Math.pow(0.92,lv-1)*mCd(p,'holy');
        for(let i=0;i<n;i++){
          let tx,ty;
          ...
          B.zones.push({x:tx, y:ty, r:(evo?72:34+3*lv)*areaMult(p)*mR(p,'holy'),
            t:0, life:evo?6:3.0, dmg:(evo?9:3+1*(lv-1))*mDmg(p,'holy'), tick:0, evo});
...
function orbPos(i,n){
  ...
  const R=((evo?70:56)+4*Math.max(1,orbN(p)))*areaMult(p)*mR(p,'orb');
...
function readyEvos(){
  ...
  for(const k in EVOS){
    const e=EVOS[k];
    if(!p.evo[k] && p.wp[e.base]>=EVO_LV && p.ps[e.pair]>=2) out.push(k);
  }
  return out;
}
...
function offerLevelup(){
  ...
  if(!pool.length){
    /* すべて上限に達したあと。ここでレベルが捨てられていた(回復するだけ)ので、
       「ひかりの余剰」として積み上がる恒久強化にした。何レベル上げても無駄にならない。 */
    p.surplus=(p.surplus||0)+1;
    p.maxHp+=12; p.hp=Math.min(p.maxHp,p.hp+40);
    p.dmgMult=(p.dmgMult||1)*1.04;
    setBanner('ひかりの余剰 '+genNum(p.surplus), '最大HP +12 / 与ダメージ +4%', '#8fd3ff');
    floatTxt(p.x,p.y-64,'ひかりの余剰!','#8fd3ff',13,1.5);
    S.lvup();
    return;
  }
    ...
    if(p.wp.orb>0 && e.orbCd<=0){
      ...
      for(let i=0;i<n;i++){
        ...
        if(Math.hypot(e.x-o.x,(e.y-e.r)-o.y)<e.r+(evo?14:11)){
          damageEnemy(e,(evo?16:11+4*(Math.min(EVO_LV,p.wp.orb)-1))*mDmg(p,'orb'));
...
function picksTick(dt){
  ...
  for(const pk of B.picks){ if(!pk.dead && !pk.known && inSight(pk,p)){ pk.known=true; if(B.time-B.seeToastT>2.5){ B.seeToastT=B.time; floatTxt(pk.x,pk.y-30,'みつけた: '+PICK_DEF[pk.kind].name,'#8fd3ff',11,1.5); } } }
...
function startEvent(){
  ...
  heroBubble(p,pickRand(['……ひかりの、はしら? いってみる','あっち、なにかおきてる']),false,1);
