'use strict';
/* Game4 quality patch v1.2 — AI/telemetry/rendering only.
...
(function(){
 ...
 function beamEscape(p){
  ...
  if(typeof crestKnow!=='function'||crestKnow()>0)for(const b of(G.B.ebullets||[])){const sp=Math.hypot(b.vx,b.vy);if(!sp)continue;const q=lineDodge(p,b.x,b.y,Math.atan2(b.vy,b.vx),240,52);if(q)return q}
  return null;
 }
 ...
 window.Game4QualityPatch={version:'1.2.0',audit:()=>G&&G.B?(G.B.tacticalAudit||[]):[],samples:()=>G&&G.B?(G.B.balanceAudit||[]):[],joinAudit:()=>G&&G.B?(G.B.joinAudit||[]):[]};
})();